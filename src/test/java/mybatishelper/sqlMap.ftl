...
<#else>
		...
		<trim prefixOverrides="AND | OR">
			<if test="@com.boda.core.Ognl@isNotEmpty(id)">  
...
<#list insert.sqlMapColumnList as column>
<#if column.isSimpleType()>
			<if test="@com.boda.core.Ognl@isNotEmpty(${column.propertyName})">  
	            AND ${select.simpleTableName}.${column.columnName} = ${r"#{"}${column.propertyName}${r"}"}
	        </if>
<#else>
			<if test="${column.propertyName} != null">
				<if test="@com.boda.core.Ognl@isNotEmpty(${column.propertyName}.${column.joinPropertyName})">  
		...
		<choose>  
	        <when test="@com.boda.core.Ognl@isNotEmpty(orderSql)">  
