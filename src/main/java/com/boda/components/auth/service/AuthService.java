...
import com.boda.components.auth.dao.AuthDao;
import com.boda.components.auth.model.AuthItem;
...
public class AuthService implements ApplicationContextAware {
	 ...
	 * @see [类、类#方法、类#成员]
	 */
	public List<AuthItemRef> queryAuthItemRefList(String operatorId) {
		if (StringUtils.isEmpty(operatorId)) {
			throw new ParameterIsEmptyException(
					"queryAuthItemRefListByOperatorId(operatorId) operatorId is empty.");
		}

		List<AuthItemRef> authItemRefList = this.authDao
	...
	public void loadAuthItemConfig() {
		...
		try {
			...
			logger.error("加载权限配置异常信息配置路径为:{}异常信息{}", this.authConfigLocaions,
					e.toString());
	...
	private void loadAuthItemConfig(Map<String, AuthItem> authItemMap,
		...
		List<Element> authElList = parentElement
				.elements(AuthConstant.AUTH_ELEMENT_NAME);
		...
		for (Element authElTemp : authElList) {
			// 读取权限配置的属性值
			String id = authElTemp
					.attributeValue(AuthConstant.AUTH_ELEMENT_ATTR_ID);
			String name = authElTemp
					.attributeValue(AuthConstant.AUTH_ELEMENT_ATTR_NAME);
			String authType = authElTemp
					.attributeValue(AuthConstant.AUTH_ELEMENT_ATTR_AUTHTYPE);
			String description = authElTemp
					.attributeValue(AuthConstant.AUTH_ELEMENT_ATTR_DESCRIPTION);
			...
			if (authItemMap.containsKey(id)) {
				...
				// 如果该权限原不存在则新生成
				newAuthItem = parentAuthItem.createChildAuthItem(id, authType,
