...
public class DistrictService extends AbstractBasicDataService<District> {
    ...
    public void insert(District district) {
        ...
        AssertUtils.notEmpty(district.getZipCode(), "district.zipCode is empty.");
        
        //为添加的数据需要填入默认值的字段填入默认值
        if (district.getParent() == null) {
            district.setLevel(0);
        } else {
            AssertUtils.notEmpty(district.getParent().getId(), "ditrict.parent.id is empty.");
     ...
     * @see [类、类#方法、类#成员]
     */
    public PagedList<District> queryPagedList(Boolean valid, Map<String, Object> params, int pageIndex, int pageSize) {
        //T判断条件合法性
        ...
        //根据实际情况，填入排序字段等条件，根据是否需要排序，选择调用dao内方法
        PagedList<District> resPagedList = this.districtDao.queryPagedList(params, pageIndex, pageSize);
