 ...
 * @since  [产品/模块版本]
*/
@Component("dddSettleStrategy")
public class DDDSettleStrategy extends AbstractSettleStrategy {
     ...
     */
    @Override
    public LATradingRecord settle(LoanAccount loanAccount, SettleIntention settleIntention) {
        ...
        AssertUtils.notEmpty(settleIntention.getBankAccountId(), "settleIntention.bankAccountId is empty.");
        
        AssertUtils.notNull(settleIntention.getRepayDate(), "settleIntention.repayDate is null.");
        AssertUtils.notNull(settleIntention.getRepayAmount(), "settleIntention.amount is null.");
        
        String loanAccountId = loanAccount.getId();
        AssertUtils.isTrue(loanAccountId.equals(settleIntention.getLoanAccountId()), "loanAccountId is not matches.");
        
        SettleRequest rRequest = new SettleRequest(loanAccountId, settleIntention);
        CommandResponse response = CommandContext.getContext().post(rRequest);
        
        LATradingRecord tr = (LATradingRecord) response.getAttribute(LoanAccountConstants.RESPONSE_KEY_TRADINGRECORD);
        return tr;
    }
     ...
     */
    @Override
    public List<ChargeRecordEntry> buildSettleChargeEntryList(LoanAccount loanAccount,
        ...
        //计算提前结清计费
        List<ChargeRecordEntry> creListWithTQHKWYJ = doBuildEarlySettleChargeEntryList(loanAccount,
                feeItemMap,
                feeItemCfgMap,
                handler,
                tradingRecord,
     ...
     * @see [类、类#方法、类#成员]
     */
    protected List<ChargeRecordEntry> doBuildEarlySettleChargeEntryList(LoanAccount loanAccount,
            Map<FeeItemEnum, LoanAccountFeeItem> feeItemMap, Map<FeeItemEnum, FeeItemCfg> feeItemCfgMap,
            PaymentScheduleHandler handler, LATradingRecord tradingRecord, Date repayDate, int receivablePeriod, int scale) {
        ...
        for (PaymentSchedule psTemp : handler.getPaymentScheduleMap().valueList()) {
            String period = psTemp.getPeriod();
            if (!NumberUtils.isDigits(period)) {
                continue;
            }
            if (NumberUtils.isDigits(period) && NumberUtils.toInt(period) <= receivablePeriod) {
                //小于应收期数的时候无需进行冲抵
                continue;
            }
            ...
            List<PaymentScheduleEntry> pseList = psTemp.getPaymentScheduleEntryList();
            for (PaymentScheduleEntry pse : pseList) {
                ...
                creList.add(ChargeRecordEntryHelper.buildBalanceEntryList(tradingRecord, pse));
            }
        }
        
        //=receivablePeriod的时候计算应收
        for (PaymentSchedule psTemp : handler.getPaymentScheduleMap().valueList(String.valueOf(receivablePeriod))) {
            ScheduleTypeEnum scheduleType = psTemp.getScheduleType();
            String period = psTemp.getPeriod();
            List<PaymentScheduleEntry> pseList = psTemp.getPaymentScheduleEntryList();
            ...
            if (!StringUtils.isEmpty(psTemp.getPrePeriod())
                    && handler.getPaymentScheduleMap().get(scheduleType, psTemp.getPrePeriod()).getRepaymentDate() != null) {
                //存在上一期，并且上一期的还款日不为空
                prePeriodRepayDate = new DateTime(
                        handler.getPaymentScheduleMap().get(scheduleType, psTemp.getPrePeriod()).getRepaymentDate());
            } else {
                prePeriodRepayDate = new DateTime(loanAccount.getInterestDate());
            }
            ...
            for (PaymentScheduleEntry pse : pseList) {
                ...
                AssertUtils.notNull(psTemp.getRepaymentDate(), "ps.repaymentDate is null.");
                BigDecimal receivableAmountTemp = pse.getReceivableAmount();
                BigDecimal sumTemp = pse.getExemptAmount().abs().add(pse.getActualReceivedAmount());
                
                if (DateUtils.compareByDay(repayDate, prePeriodRepayDate.toDate()) < 0) {
                    receivableAmountTemp = sumTemp;
                } else if (DateUtils.compareByDay(repayDate, psTemp.getRepaymentDate()) < 0) {
                    //不到一个月
                    int daysTemp = DateUtils.calculateNumberOfDaysBetween(repayDate, prePeriodRepayDate.toDate()) + 1;
                    ...
                    receivableAmountTemp = amountTemp.compareTo(sumTemp) >= 0 ? amountTemp : sumTemp;
                } else {
                    receivableAmountTemp = pse.getReceivableAmount();
                }
                
                if (receivableAmountTemp.compareTo(pse.getReceivableAmount()) < 0) {
                    //增加本金类型的费用项的应收
                    creList.add(ChargeRecordEntryHelper.buildChargeRecordEntry(tradingRecord,
                            handler,
                            scheduleType,
                            period,
                            feeItem,
                            receivableAmountTemp.subtract(pse.getReceivableAmount())));
                }
            }
        }
        //写入>当前期数时，将应收写入当前期数的情形
        for (Entry<ScheduleTypeEnum, Map<FeeItemEnum, BigDecimal>> entryTemp : sumMap.entrySet()) {
            ScheduleTypeEnum scheduleTypeTemp = entryTemp.getKey();
            for (Entry<FeeItemEnum, BigDecimal> eTemp : entryTemp.getValue().entrySet()) {
        ...
        //提前结清手续费
        if (feeItemMap.get(FeeItemEnum.DK_TQHKWYJ) == null
                || StringUtils.isEmpty(feeItemMap.get(FeeItemEnum.DK_TQHKWYJ).getValue())) {
            return creList;
        }
        String tqhksxfRateValue = feeItemMap.get(FeeItemEnum.DK_TQHKWYJ).getValue();
        BigDecimal tqhksxfRate = new BigDecimal(tqhksxfRateValue);
        BigDecimal tqhksxf = handler.getLoanAccount().getLoanAmount().multiply(tqhksxfRate).setScale(2, RoundingMode.UP);
     ...
     * @see [类、类#方法、类#成员]
     */
    protected List<ChargeRecordEntry> doBuildBalanceSettleChargeEntryList(Map<FeeItemEnum, LoanAccountFeeItem> feeItemMap,
            Map<FeeItemEnum, FeeItemCfg> feeItemCfgMap, PaymentScheduleHandler handler, LATradingRecord tradingRecord,
        ...
        for (PaymentSchedule psTemp : handler.getPaymentScheduleMap().valueList()) {
            ...
            for (PaymentScheduleEntry pse : pseList) {
                FeeItemEnum feeItem = pse.getFeeItem();
                BigDecimal sumTemp = pse.getReceivableAmount().add(pse.getExemptAmount()).subtract(pse.getActualReceivedAmount());
        ...
        for (PaymentSchedule psTemp : handler.getPaymentScheduleMap().valueList(String.valueOf(receivablePeriod))) {
            ...
            for (PaymentScheduleEntry pse : pseList) {
                ...
                if (DateUtils.compareByDay(repayDate, dt.toDate()) < 0) {
                    ...
                    //不到一个月
                    int daysTemp = DateUtils.calculateNumberOfDaysBetween(repayDate, dt.toDate()) + 1;
                    BigDecimal amountTemp = pse.getReceivableAmount()
                            .divide(new BigDecimal(days), 8, RoundingMode.UP)
                            .multiply(new BigDecimal(daysTemp))
                ...
                AssertUtils.isTrue(receivableAmountTemp.compareTo(pse.getReceivableAmount()) < 0,
                        "receivableAmountTemp:{} should < pse.getReceivableAmount():{}.",
                        new Object[] { receivableAmountTemp, pse.getReceivableAmount() });
                //增加本金类型的费用项的应收
                creList.add(ChargeRecordEntryHelper.buildChargeRecordEntry(tradingRecord,
                        handler,
                        scheduleType,
