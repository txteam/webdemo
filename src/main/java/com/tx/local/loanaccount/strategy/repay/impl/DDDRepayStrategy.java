 ...
 * @since  [产品/模块版本]
 */
@Component("dddRepayStrategy")
public class DDDRepayStrategy extends AbstractRepayStrategy {
    ...
    protected List<ChargeRecordEntry> doBuildRepayChargeEntryList(
        ...
        for (PaymentSchedule psTemp : handler.getPaymentScheduleMap()
            ...
            for (PaymentScheduleEntry pse : pseList) {
                ...
                BigDecimal receivableAmount = psEntry.getReceivableAmount()
                ...
                if (DateUtils.compareByDay(repayDate,
                    ...
                    receivableAmountTemp = sumTemp;
                        ...
                        && receivableAmount.compareTo(BigDecimal.ZERO) == 1) {
                    //不到一个月
                    ...
                    //月利率
                    BigDecimal rRate = yearRate.divide(new BigDecimal(360),
                            8,
                            RoundingMode.UP);
                    BigDecimal rlx = principalBalance.multiply(rRate)
                            .setScale(scale, BigDecimal.ROUND_UP);
                    BigDecimal amountTemp = rlx
        ...
        for (ChargeRecordEntry chargeRecordEntry : chargeRecordEntryList) {
            ...
            if (!feeItemCfgMap.get(chargeRecordEntry.getFeeItem()).isPrincipal()
                ...
                if (amount.compareTo(BigDecimal.ZERO) == 1) {
                    ...
                    //月利率
                    BigDecimal monthlyRate = yearRate
                            .divide(new BigDecimal(360), 8, RoundingMode.UP);
