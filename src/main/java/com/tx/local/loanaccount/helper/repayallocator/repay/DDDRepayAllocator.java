...
/**
  ...
  * @since  [产品/模块版本]
 */
public class DDDRepayAllocator extends AbstractRepayAllocator {
    
    /** <默认构造函数> */
    public DDDRepayAllocator(LoanAccount loanAccount, PaymentScheduleHandler handler, RepayIntention repayIntention) {
    ...
    protected Comparator<AssignEntryComparable> buildAssignComparator(ScheduleTypeEnum scheduleType) {
        Comparator<AssignEntryComparable> comparator = new RepayAssignComparator(loanAccount, feeItemCfgMap, this.handler,
                scheduleType, repayDate);
        return comparator;
    }
