...
/**
  ...
  * @since  [产品/模块版本]
  */
public class AbstractRepayReceiver<PR extends AbstractRepayRequest> extends AbstractDefaultTradingReceiver<PR> {
    ...
    public boolean preHandle(PR request, CommandResponse response) {
        ...
        switch (loanAccount.getAccountStatus()) {
            ...
            case FP:
                ...
                //如果已经到账的金额，发起超额还款
                OverRepayRequest overRepayRequest = new OverRepayRequest(loanAccountId, request.getRepayIntention(),
    ...
    public List<PaymentRecordEntry> createPaymentRecordEntryList(PR request, CommandResponse response,
        ...
        RepayAllocator repayAllocator = getRepayAllocator(loanAccount, handler, request.getRepayIntention(), response);
        //还款分配
        List<PaymentRecordEntry> preList = strategy.assignOfRepay(tradingRecord,
                loanAccount,
                handler,
                repayAllocator,
                true);
    ...
    public void afterHandle(PR request, CommandResponse response, LoanAccount loanAccount,
        ...
        //修改此处还款事件中的金额，使用交易金额
        RepayEvent event = new RepayEvent(tradingRecord.getSum(), loanAccount, tradingRecord, handler, paymentRecords,
                request);
     ...
     * @see [类、类#方法、类#成员]
     */
    protected RepayAllocator getRepayAllocator(LoanAccount loanAccount, PaymentScheduleHandler handler,
            RepayIntention repayIntention, CommandResponse response) {
        AssertUtils.notNull(loanAccount, "loanAccount is null.");
        ...
        RepayAllocator repayAllocator = (RepayAllocator) response
                .getAttribute(LoanAccountConstants.RESPONSE_KEY_REPAY_ALLOCATOR);
        if (repayAllocator == null) {
            LoanAccountStrategy strategy = LoanAccountStrategyHelper.getStrategy(loanAccount);
            repayAllocator = strategy.assignOfRepay(loanAccount, handler, repayIntention);
            
            response.setAttribute(LoanAccountConstants.RESPONSE_KEY_REPAY_ALLOCATOR, repayAllocator);
        }
