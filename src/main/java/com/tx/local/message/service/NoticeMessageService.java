/*
 * 描          述:  <描述>
 * 修  改   人:  
 * 修改时间:  
 * <修改描述:>
 */
package com.tx.local.message.service;

import java.util.*;

import javax.annotation.Resource;

import com.tx.local.message.dao.Notice2UserDao;
import com.tx.local.message.model.MessageUserTypeEnum;
import com.tx.local.message.model.Notice2User;
import com.tx.local.message.model.NoticeMessageDetail;
import com.tx.local.operator.dao.OperatorDao;
import com.tx.local.operator.model.Operator;
import com.tx.local.security.util.WebContextUtils;
import org.apache.commons.collections4.CollectionUtils;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.stereotype.Component;
import org.springframework.transaction.annotation.Transactional;

import com.tx.local.message.dao.NoticeMessageDao;
import com.tx.local.message.model.NoticeMessage;
import com.tx.core.exceptions.util.AssertUtils;
import com.tx.core.paged.model.PagedList;
import com.tx.core.querier.model.Filter;
import com.tx.core.querier.model.Querier;
import com.tx.core.querier.model.QuerierBuilder;

/**
 * 公告消息的业务层[NoticeMessageService]
 * <功能详细描述>
 * 
 * @author  
 * @version [版本号]
 * @see [相关类/方法]
 * @since [产品/模块版本]
 */
@Component("noticeMessageService")
public class NoticeMessageService {
    
    @SuppressWarnings("unused")
    private Logger logger = LoggerFactory.getLogger(NoticeMessageService.class);
    
    @Resource(name = "noticeMessageDao")
    private NoticeMessageDao noticeMessageDao;

    @Resource(name = "notice2UserDao")
    private Notice2UserDao notice2UserDao;

    @Resource(name = "operatorDao")
    private OperatorDao operatorDao;
    
    /**
     * 新增公告消息实例<br/>
     * 将noticeMessage插入数据库中保存
     * 1、如果noticeMessage 为空时抛出参数为空异常
     * 2、如果noticeMessage 中部分必要参数为非法值时抛出参数不合法异常
     * 
     * @param noticeMessage [参数说明]
     * @return void [返回类型说明]
     * @exception throws
     * @see [类、类#方法、类#成员]
     */
    @Transactional
    public void insert(NoticeMessage noticeMessage) {
        noticeMessage.setVcid(WebContextUtils.getVcid());
        //验证参数是否合法
        AssertUtils.notNull(noticeMessage, "noticeMessage is null.");
		AssertUtils.notEmpty(noticeMessage.getVcid(), "noticeMessage.vcid is empty.");
		AssertUtils.notEmpty(noticeMessage.getTitle(), "noticeMessage.title is empty.");
           
        //FIXME:为添加的数据需要填入默认值的字段填入默认值
		noticeMessage.setLastUpdateDate(new Date());
		noticeMessage.setValid(true);
		noticeMessage.setCreateDate(new Date());
		noticeMessage.setLastUpdateUserId(WebContextUtils.getOperatorId());
		noticeMessage.setCreateUserId(WebContextUtils.getOperatorId());

        //调用数据持久层对实例进行持久化操作
        this.noticeMessageDao.insert(noticeMessage);
    }
    
    /**
     * 根据id删除公告消息实例
     * 1、如果入参数为空，则抛出异常
     * 2、执行删除后，将返回数据库中被影响的条数 > 0，则返回true
     *
     * @param id
     * @return boolean 删除的条数>0则为true [返回类型说明]
     * @exception throws 
     * @see [类、类#方法、类#成员]
     */
    @Transactional
    public boolean deleteById(String id) {
        AssertUtils.notEmpty(id, "id is empty.");
        
        NoticeMessage condition = new NoticeMessage();
        condition.setId(id);
        
        int resInt = this.noticeMessageDao.delete(condition);
        boolean flag = resInt > 0;
        return flag;
    }
    
    /**
     * 根据id查询公告消息实例
     * 1、当id为empty时抛出异常
     *
     * @param id
     * @return NoticeMessage [返回类型说明]
     * @exception throws
     * @see [类、类#方法、类#成员]
     */
    public NoticeMessage findById(String id) {
        AssertUtils.notEmpty(id, "id is empty.");
        
        NoticeMessage condition = new NoticeMessage();
        condition.setId(id);
        
        NoticeMessage res = this.noticeMessageDao.find(condition);
        return res;
    }
    
    /**
     * 查询公告消息实例列表
     * <功能详细描述>
     * @param valid
     * @param params      
     * @return [参数说明]
     * 
     * @return List<NoticeMessage> [返回类型说明]
     * @exception throws [异常类型] [异常说明]
     * @see [类、类#方法、类#成员]
     */
    public List<NoticeMessage> queryList(
		Boolean valid,
		Map<String,Object> params   
    	) {
        //判断条件合法性
        
        //生成查询条件
        params = params == null ? new HashMap<String, Object>() : params;
		params.put("valid",valid);

        //根据实际情况，填入排序字段等条件，根据是否需要排序，选择调用dao内方法
        List<NoticeMessage> resList = this.noticeMessageDao.queryList(params);
        
        return resList;
    }
    
    /**
     * 查询公告消息实例列表
     * <功能详细描述>
     * @param valid
     * @param querier      
     * @return [参数说明]
     * 
     * @return List<NoticeMessage> [返回类型说明]
     * @exception throws [异常类型] [异常说明]
     * @see [类、类#方法、类#成员]
     */
    public List<NoticeMessage> queryList(
		Boolean valid,
		Querier querier   
    	) {
        //判断条件合法性
        
        //生成查询条件
        querier = querier == null ? QuerierBuilder.newInstance().querier()
                : querier;
		if (valid != null) {
            querier.getFilters().add(Filter.eq("valid", valid));
        }

        //根据实际情况，填入排序字段等条件，根据是否需要排序，选择调用dao内方法
        List<NoticeMessage> resList = this.noticeMessageDao.queryList(querier);
        
        return resList;
    }
    
    /**
     * 分页查询公告消息实例列表
     * <功能详细描述>
     * @param valid
     * @param params    
     * @param pageIndex 当前页index从1开始计算
     * @param pageSize 每页显示行数
     * 
     * <功能详细描述>
     * @return [参数说明]
     * 
     * @return List<NoticeMessage> [返回类型说明]
     * @exception throws [异常类型] [异常说明]
     * @see [类、类#方法、类#成员]
     */
    public PagedList<NoticeMessage> queryPagedList(
		Boolean valid,
		Map<String,Object> params,
    	int pageIndex,
        int pageSize) {
        //T判断条件合法性
        
        //生成查询条件
        params = params == null ? new HashMap<String, Object>() : params;
		params.put("valid",valid);
 
        //根据实际情况，填入排序字段等条件，根据是否需要排序，选择调用dao内方法
        PagedList<NoticeMessage> resPagedList = this.noticeMessageDao.queryPagedList(params, pageIndex, pageSize);
        
        return resPagedList;
    }

    public PagedList<NoticeMessageDetail> queryPagedList(
            Map<String,Object> params,
            int pageIndex,
            int pageSize) {
        Integer offset = (pageIndex - 1) * pageSize;
        params.put("rows",pageSize);
        params.put("offset",offset);
        List<NoticeMessageDetail> list = noticeMessageDao.queryPagedListByCondition(params);
        Integer count = noticeMessageDao.queryCountByCondition(params);
        PagedList<NoticeMessageDetail> pagedList = new PagedList<>();
        pagedList.setList(list);
        pagedList.setCount(count);
        return pagedList;
    }
    
	/**
     * 分页查询公告消息实例列表
     * <功能详细描述>
     * @param valid
     * @param querier    
     * @param pageIndex 当前页index从1开始计算
     * @param pageSize 每页显示行数
     * 
     * <功能详细描述>
     * @return [参数说明]
     * 
     * @return List<NoticeMessage> [返回类型说明]
     * @exception throws [异常类型] [异常说明]
     * @see [类、类#方法、类#成员]
     */
    public PagedList<NoticeMessage> queryPagedList(
		Boolean valid,
		Querier querier,
    	int pageIndex,
        int pageSize) {
        //T判断条件合法性
        
        //生成查询条件
        querier = querier == null ? QuerierBuilder.newInstance().querier()
                : querier;
		if (valid != null) {
            querier.getFilters().add(Filter.eq("valid", valid));
        }
 
        //根据实际情况，填入排序字段等条件，根据是否需要排序，选择调用dao内方法
        PagedList<NoticeMessage> resPagedList = this.noticeMessageDao.queryPagedList(querier, pageIndex, pageSize);
        
        return resPagedList;
    }
    
    /**
     * 查询公告消息实例数量<br/>
     * <功能详细描述>
     * @param valid
     * @param params      
     * @return [参数说明]
     * 
     * @return List<NoticeMessage> [返回类型说明]
     * @exception throws [异常类型] [异常说明]
     * @see [类、类#方法、类#成员]
     */
    public int count(
		Boolean valid,
		Map<String,Object> params   
    	) {
        //判断条件合法性
        
        //生成查询条件
        params = params == null ? new HashMap<String, Object>() : params;
		params.put("valid",valid);

        //根据实际情况，填入排序字段等条件，根据是否需要排序，选择调用dao内方法
        int res = this.noticeMessageDao.count(params);
        
        return res;
    }
    
    /**
     * 查询公告消息实例数量<br/>
     * <功能详细描述>
     * @param valid
     * @param querier      
     * @return [参数说明]
     * 
     * @return List<NoticeMessage> [返回类型说明]
     * @exception throws [异常类型] [异常说明]
     * @see [类、类#方法、类#成员]
     */
    public int count(
		Boolean valid,
		Querier querier   
    	) {
        //判断条件合法性
        
        //生成查询条件
        querier = querier == null ? QuerierBuilder.newInstance().querier()
                : querier;
		if (valid != null) {
            querier.getFilters().add(Filter.eq("valid", valid));
        }

        //根据实际情况，填入排序字段等条件，根据是否需要排序，选择调用dao内方法
        int res = this.noticeMessageDao.count(querier);
        
        return res;
    }
    
    /**
     * 判断公告消息实例是否已经存在<br/>
     * <功能详细描述>
     * @param key2valueMap
     * @param excludeId
     * @return
     * 
     * @return int [返回类型说明]
     * @exception throws [异常类型] [异常说明]
     * @see [类、类#方法、类#成员]
     */
    public boolean exists(Map<String,String> key2valueMap, String excludeId) {
        AssertUtils.notEmpty(key2valueMap, "key2valueMap is empty");
        
        //生成查询条件
        Map<String, Object> params = new HashMap<String, Object>();
        params.putAll(key2valueMap);
        
        //根据实际情况，填入排序字段等条件，根据是否需要排序，选择调用dao内方法
        int res = this.noticeMessageDao.count(params,excludeId);
        
        return res > 0;
    }
    
    /**
     * 判断公告消息实例是否已经存在<br/>
     * <功能详细描述>
     * @param key2valueMap
     * @param excludeId
     * @return
     * 
     * @return int [返回类型说明]
     * @exception throws [异常类型] [异常说明]
     * @see [类、类#方法、类#成员]
     */
    public boolean exists(Querier querier, String excludeId) {
        AssertUtils.notNull(querier, "querier is null.");
        
        //根据实际情况，填入排序字段等条件，根据是否需要排序，选择调用dao内方法
        int res = this.noticeMessageDao.count(querier,excludeId);
        
        return res > 0;
    }
    
    /**
     * 根据id更新公告消息实例<br/>
     * <功能详细描述>
     * @param noticeMessage
     * @return [参数说明]
     * 
     * @return boolean [返回类型说明]
     * @exception throws [异常类型] [异常说明]
     * @see [类、类#方法、类#成员]
     */
    @Transactional
    public boolean updateById(String id,NoticeMessage noticeMessage) {
        //验证参数是否合法，必填字段是否填写
        AssertUtils.notNull(noticeMessage, "noticeMessage is null.");
        AssertUtils.notEmpty(id, "id is empty.");
		AssertUtils.notEmpty(noticeMessage.getTitle(), "noticeMessage.title is empty.");

        //生成需要更新字段的hashMap
        Map<String, Object> updateRowMap = new HashMap<String, Object>();
        //FIXME:需要更新的字段
		updateRowMap.put("lastUpdateUserId", WebContextUtils.getOperatorId());
		updateRowMap.put("title", noticeMessage.getTitle());
		updateRowMap.put("sender", noticeMessage.getSender());
		updateRowMap.put("content", noticeMessage.getContent());
		updateRowMap.put("priority", noticeMessage.getPriority());
		updateRowMap.put("attributes", noticeMessage.getAttributes());
		updateRowMap.put("lastUpdateDate", new Date());

        boolean flag = this.noticeMessageDao.update(id,updateRowMap); 
        //如果需要大于1时，抛出异常并回滚，需要在这里修改
        return flag;
    }
    
    /**
     * 根据id更新公告消息实例<br/>
     * <功能详细描述>
     * @param noticeMessage
     * @return [参数说明]
     * 
     * @return boolean [返回类型说明]
     * @exception throws [异常类型] [异常说明]
     * @see [类、类#方法、类#成员]
     */
    @Transactional
    public boolean updateById(NoticeMessage noticeMessage) {
        //验证参数是否合法，必填字段是否填写
        AssertUtils.notNull(noticeMessage, "noticeMessage is null.");
        AssertUtils.notEmpty(noticeMessage.getId(), "noticeMessage.id is empty.");

        boolean flag = updateById(noticeMessage.getId(),noticeMessage); 
        //如果需要大于1时，抛出异常并回滚，需要在这里修改
        return flag;
    }

    /**
     * 根据id禁用公告消息<br/>
     * <功能详细描述>
     * @param id
     * @return [参数说明]
     * 
     * @return boolean [返回类型说明]
     * @exception throws [异常类型] [异常说明]
     * @see [类、类#方法、类#成员]
     */
    @Transactional
    public boolean disableById(String id) {
        AssertUtils.notEmpty(id, "id is empty.");
        
        //生成条件
        Map<String, Object> params = new HashMap<String, Object>();
        params.put("id", id);
        params.put("valid", false);
		params.put("lastUpdateDate", new Date());
        params.put("lastUpdateUserId",WebContextUtils.getOperatorId());
	        
        boolean flag = this.noticeMessageDao.update(params) > 0;
        
        return flag;
    }
    
    /**
     * 根据id启用公告消息<br/>
     * <功能详细描述>
     * @param id
     * @return [参数说明]
     * 
     * @return boolean [返回类型说明]
     * @exception throws [异常类型] [异常说明]
     * @see [类、类#方法、类#成员]
     */
    @Transactional
    public boolean enableById(String id) {
        AssertUtils.notEmpty(id, "id is empty.");
        
        //生成查询条件
        Map<String, Object> params = new HashMap<String, Object>();
        params.put("id", id);
        params.put("valid", true);
		params.put("lastUpdateDate", new Date());
		params.put("lastUpdateUserId",WebContextUtils.getOperatorId());

        boolean flag = this.noticeMessageDao.update(params) > 0;
        
        return flag;
    }

    @Transactional
    public boolean readAll() {

        Map<String, Object> queryParams = new HashMap<String, Object>();
        queryParams.put("userId",WebContextUtils.getOperatorId());
        queryParams.put("unread",true);
        List<Notice2User> notice2Users = this.notice2UserDao.queryList(queryParams);

        List<Map<String, Object>> list = new ArrayList<>();
        if(CollectionUtils.isNotEmpty(notice2Users)){
            for (Notice2User notice2User : notice2Users) {
                Map<String, Object> params = new HashMap<String, Object>();
                params.put("id",notice2User.getId());
                params.put("unread", false);
                params.put("readDate",new Date());
                list.add(params);
            }
            this.notice2UserDao.batchUpdate(list);
        }

        return true;
    }

    @Transactional
    public boolean publishById(String id) {
        AssertUtils.notEmpty(id, "id is empty.");

        Map<String,Object> operatorParams = new HashMap<>();
        operatorParams.put("vcid",WebContextUtils.getVcid());
        List<Operator> operators = operatorDao.queryList(operatorParams);
        if(CollectionUtils.isNotEmpty(operators)){
            List<Notice2User> list = new ArrayList();
            for (Operator operator : operators) {
                Notice2User notice2User = new Notice2User();
                notice2User.setNoticeId(id);
                notice2User.setUserId(operator.getId());
                notice2User.setUnread(true);
                notice2User.setTop(false);
                notice2User.setUserType(MessageUserTypeEnum.OPERATOR);
                list.add(notice2User);
            }
            notice2UserDao.batchInsert(list);
        }

        //生成查询条件
        Map<String, Object> params = new HashMap<String, Object>();
        params.put("id", id);
        params.put("published",true);
        params.put("publishDate",new Date());
        params.put("lastUpdateDate", new Date());
        params.put("lastUpdateUserId",WebContextUtils.getOperatorId());

        boolean flag = this.noticeMessageDao.update(params) > 0;

        return flag;
    }

    @Transactional
    public boolean deleteNotice2UserById(String id) {
        AssertUtils.notEmpty(id, "id is empty.");

        Notice2User condition = new Notice2User();
        condition.setId(id);

        int resInt = this.notice2UserDao.delete(condition);
        boolean flag = resInt > 0;
        return flag;
    }

    @Transactional
    public boolean readById(String id) {
        Map<String, Object> params = new HashMap<String, Object>();
        params.put("id",id);
        params.put("unread", false);
        params.put("readDate",new Date());
        boolean flag = this.notice2UserDao.update(params) > 0;
        return flag;
    }
}
