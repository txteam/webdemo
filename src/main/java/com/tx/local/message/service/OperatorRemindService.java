 ...
 * @since  [产品/模块版本]
 */
@Component("operatorRemindService")
public class OperatorRemindService {
    ...
    public void insert(String title, String content, String vcid,
        ...
        PrivateMessage rm = new PrivateMessage();
        rm.setTitle(title);
        rm.setContent(content);
        rm.setVcid(vcid);
        Date now = new Date();
        rm.setLastUpdateDate(now);
        rm.setCreateDate(now);
        
        rm.setCatalog(PrivateMessageCatalogEnum.MESSAGE);
        rm.setUserType(MsgUserTypeEnum.OPERATOR);
        rm.setCreateUserId(senderUserId);
        rm.setLastUpdateUserId(senderUserId);
        
        rm.setUserId(receiveUserId);
        rm.setType(PrivateMessageTypeEnum.RECEIVE);
        rm.setSender(sender);
        rm.setReceiver(receiveUserId);
        
        //调用数据持久层对实例进行持久化操作
        this.privateMessageService.insert(rm);
    }
