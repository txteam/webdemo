 ...
 * @since [产品/模块版本]
 */
@Component("operatorOrganizationService")
public class OperatorOrganizationService {
    
    @SuppressWarnings("unused")
    private Logger logger = LoggerFactory
            .getLogger(OperatorOrganizationService.class);
    
    @Resource(name = "operatorOrganizationDao")
    private OperatorOrganizationDao operatorOrganizationDao;
    ...
    private String generateFullName(String parentId, String organizationName) {
        ...
        if (!StringUtils.isEmpty(parentId)) {
            OperatorOrganization parent = findById(parentId);
            if (parent != null) {
                sb.insert(0, "_");
    ...
    public void insertToHis(OperatorOrganization organization) {
        ...
        //调用数据持久层对实例进行持久化操作
        this.operatorOrganizationDao.insertToHis(organization);
    }
     ...
     */
    @Transactional
    public void insert(OperatorOrganization organization) {
        //验证参数是否合法
        AssertUtils.notNull(organization, "organization is null.");
        AssertUtils.notEmpty(organization.getCode(),
                "organization.code is empty.");
        ...
        switch (organization.getType()) {
            ...
            default:
                ...
                OperatorOrganization parent = this
        ...
        //调用数据持久层对实例进行持久化操作
        this.operatorOrganizationDao.insert(organization);
    }
    ...
    public boolean deleteById(String id) {
        ...
        OperatorOrganization condition = new OperatorOrganization();
        condition.setId(id);
        
        int resInt = this.operatorOrganizationDao.delete(condition);
     ...
     * @see [类、类#方法、类#成员]
     */
    public OperatorOrganization findById(String id) {
        AssertUtils.notEmpty(id, "id is empty.");
        
        OperatorOrganization condition = new OperatorOrganization();
        condition.setId(id);
        
        OperatorOrganization res = this.operatorOrganizationDao.find(condition);
        return res;
    }
     ...
     * @see [类、类#方法、类#成员]
     */
    public OperatorOrganization findByCode(String code) {
        AssertUtils.notEmpty(code, "code is empty.");
     ...
     * @see [类、类#方法、类#成员]
     */
    public List<OperatorOrganization> queryList(Boolean valid,
        ...
        List<OperatorOrganization> resList = this.operatorOrganizationDao
                .queryList(params);
     ...
     * @see [类、类#方法、类#成员]
     */
    public PagedList<OperatorOrganization> queryPagedList(Boolean valid,
        ...
        //根据实际情况，填入排序字段等条件，根据是否需要排序，选择调用dao内方法
        PagedList<OperatorOrganization> resPagedList = this.operatorOrganizationDao
    ...
    public int count(Boolean valid, Map<String, Object> params) {
        ...
        //根据实际情况，填入排序字段等条件，根据是否需要排序，选择调用dao内方法
        int res = this.operatorOrganizationDao.count(params);
    ...
    public int count(Boolean valid, Querier querier) {
        ...
        //根据实际情况，填入排序字段等条件，根据是否需要排序，选择调用dao内方法
        int res = this.operatorOrganizationDao.count(querier);
    ...
    public boolean exists(Querier querier, String excludeId) {
        ...
        //根据实际情况，填入排序字段等条件，根据是否需要排序，选择调用dao内方法
        int res = this.operatorOrganizationDao.count(querier, excludeId);
     ...
     */
    @Transactional
    public boolean updateById(String id, OperatorOrganization organization) {
        //验证参数是否合法，必填字段是否填写
        AssertUtils.notNull(organization, "organization is null.");
        AssertUtils.notEmpty(id, "id is empty.");
        AssertUtils.notEmpty(organization.getCode(),
        ...
        boolean flag = this.operatorOrganizationDao.update(id, updateRowMap);
     ...
     */
    @Transactional
    public boolean updateById(OperatorOrganization organization) {
        //验证参数是否合法，必填字段是否填写
        AssertUtils.notNull(organization, "organization is null.");
    ...
    public boolean disableById(String id) {
        ...
        boolean flag = this.operatorOrganizationDao.update(params) > 0;
     ...
     * @see [类、类#方法、类#成员]
     */
    public List<OperatorOrganization> queryChildrenByParentId(String parentId,
    ...
    public List<OperatorOrganization> queryDescendantsByParentId(
        ...
        List<OperatorOrganization> resList = doNestedQueryChildren(valid,
                ids,
                parentIds,
                params);
        return resList;
     ...
     * @see [类、类#方法、类#成员]
     */
    private List<OperatorOrganization> doNestedQueryChildren(Boolean valid,
            Set<String> ids, Set<String> parentIds,
            Map<String, Object> params) {
        if (CollectionUtils.isEmpty(parentIds)) {
            return new ArrayList<OperatorOrganization>();
        }
        ...
        List<OperatorOrganization> resList = queryList(valid, queryParams);
        
        Set<String> newParentIds = new HashSet<>();
        for (OperatorOrganization bdTemp : resList) {
    ...
    private List<OperatorOrganization> doNestedQueryChildren(Boolean valid,
        ...
        List<OperatorOrganization> resList = queryList(valid, querierClone);
    ...
    public boolean modifyAble(String id) {
        ...
        OperatorOrganization condition = new OperatorOrganization();
        condition.setId(id);
        OperatorOrganization org = this.operatorOrganizationDao.find(condition);
        if (org == null) {
            return false;
        }
