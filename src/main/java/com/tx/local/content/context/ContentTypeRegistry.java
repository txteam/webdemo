...
public class ContentTypeRegistry implements InitializingBean,
    ...
    /** 编码对处理器的映射 */
    private Map<String, ContentInfoType> code2typeMap;
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        for (ContentTypeHandler handlerTemp : handlers) {
            ContentInfoType typeTemp = buildType(handlerTemp);
            this.code2typeMap.put(typeTemp.getCode(), typeTemp);
            this.code2handlerMap.put(typeTemp.getCode(), handlerTemp);
        }
        
        ContentInfoType typeTemp = buildType(this.defaultTypeHandler);
        this.code2typeMap.put(typeTemp.getCode(), typeTemp);
    ...
    /**
      ...
      * @see [类、类#方法、类#成员]
     */
    private ContentInfoType buildType(ContentTypeHandler handler) {
        AssertUtils.notNull(handler, "handler is null.");
        AssertUtils.notEmpty(handler.type(), "handler.type is empty.");
        
        ContentInfoType type = new ContentInfoType();
    ...
    /**
      ...
      * @see [类、类#方法、类#成员]
     */
    public ContentInfoType getTypeByCode(String typeCode) {
        if (StringUtils.isEmpty(typeCode)) {
            typeCode = this.defaultTypeHandler.type();
        }
        
        ContentInfoType cit = this.code2typeMap.get(typeCode);
        return cit;
    }
     ...
     */
    public Map<String, ContentInfoType> getCode2typeMap() {
        return code2typeMap;
    }
}
