...
import com.tx.fetch.model.PersonInfo;
  ...
  * @since  [产品/模块版本]
  */
public class PersonInfoExportExcelModel extends ExportExcelModel<PersonInfo>{

    public PersonInfoExportExcelModel(List<PersonInfo> dataList) {
        super(dataList);
    }
     ...
     */
    @Override
    protected void doWriteSheet1Row(int index, Row row, PersonInfo data) {
        int cellIndex = 0;
        row.createCell(cellIndex++).setCellValue(data.getIname());
        row.createCell(cellIndex++).setCellValue(data.getIdCardNumber());
        row.createCell(cellIndex++).setCellValue("");
        row.createCell(cellIndex++).setCellValue("法院网站");
        row.createCell(cellIndex++).setCellValue("恶意拖欠");
        row.createCell(cellIndex++).setCellValue("个人");
