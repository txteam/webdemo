...
function queryBankAccount(){
	var type = $("#type").val();
	var bankAccountTypes = ['XJ_HK','ZZ_HK','POS_HK'];
	$.post('${contextPath}/loanAccountSettle/queryBankAccountList.action',{'bankAccountType[]':bankAccountTypes,'loanAccountType':loanAccountType},function(data){
		$("#bankAccountId").empty();
		if(!data){
			 return true;
		}
...
$(function(){
	...
	$("#repayAmount").focus(function(){
		$("#submitBtn").linkbutton("disable");
	});
	if($("#repayAmount").val() * 1 > 0){
		preAutoAssign();
	}
	$("#repayAmount").numberbox({
		onChange : function(){
			preAutoAssign();
		}
	});
...
function changeRepayDate(dp){
	...
	if(isNotChange){
		...
		window.location.href = "${contextPath}/loanAccountSettle/toSettle.action?loanAccountId=" + $("#loanAccountId").val() 
...
function preAutoAssign(){
	...
	var repayAmount = $("#repayAmount").numberbox("getValue") * 1;
	repayAmount = repayAmount.toFixed(2);
	if(repayAmount <= 0){
		DialogUtils.alert("提醒","交易金额应大于0.");
		return ;
	}
	var settleSum = $("#settleSum").val() * 1;
	settleSum = settleSum.toFixed(2);
	var exemptAbleSum = $("#exemptAbleSum").val() * 1;
	exemptAbleSum = exemptAbleSum.toFixed(2);
	
	var autoAssignSum = 0.0;
	var feeItem2AmountMap = {};
	//还款金额小于结清金额，并且需填入的豁免金额大于豁免金额时需要修正值
	if(repayAmount < settleSum){
		$("#exemptSum").numberbox("setValue",settleSum - repayAmount);
	}
	exemptSum = $("#exemptSum").numberbox("getValue") * 1;
	exemptSum = exemptSum.toFixed(2);
	if(exemptSum > exemptAbleSum){
		DialogUtils.alert("提醒","豁免金额:[" + exemptSum + "]大于最大可豁免金额:[" + exemptAbleSum + "]");
		return ;
	}
	if(autoAssignSum * 1 > exemptSum * 1){
		DialogUtils.alert("提醒","分配金额之和应小于或等于豁免金额.分配金额和:" + autoAssignSum + "豁免金额:" + exemptSum,"warning");
		return ;
	}
	
	//var assignSumBalance = repayAmount - autoAssignSum;
	//assignSumBalance = assignSumBalance.toFixed(2);
	//$("#assignBalanceAmount").val(assignSumBalance);//分配结余，自由分配金额
	//$("#exemptAbleSum").val(autoAssignSum);
	...
	$.post('${contextPath}/loanAccountSettle/autoAssign.action', {
...
function autoAssign(){
	...
	var autoAssignSum = 0 * 1;
	var feeItem2AmountMap = {};
	$.each(autoAssignItems,function(index,autoAssignItems){
		if(!$.ObjectUtils.isEmpty($(this).val())){
	...
	//如果分配金额>0则填入豁免金额
	if(autoAssignSum > 0){
		$("#exemptSum").numberbox("setValue",autoAssignSum);
	}
	//还款金额小于结清金额，并且需填入的豁免金额大于豁免金额时需要修正值
	if(repayAmount < settleSum && (settleSum - repayAmount) > autoAssignSum){
		$("#exemptSum").numberbox("setValue",settleSum - repayAmount);
	}
	exemptSum = $("#exemptSum").numberbox("getValue") * 1;
	exemptSum = exemptSum.toFixed(2);
	if(exemptSum * 1 > exemptAbleSum * 1){
		DialogUtils.alert("提醒","豁免金额:[" + exemptSum + "]大于最大可豁免金额:[" + exemptAbleSum + "]");
		return ;
	}
	if(autoAssignSum * 1 > exemptSum * 1){
...
function submitFun(){
	...
	$("#submitBtn").linkbutton("enable"); 
	$.post('${contextPath}/loanAccountSettle/settle.action', {
		loanAccountId : loanAccountId,
		...
		DialogUtils.progress('close');
		if (data) {
			parent.DialogUtils.tip("结清成功");
			parent.DialogUtils.closeDialogById("repay");
		} else {
			parent.DialogUtils.tip("结清失败");
		}
	});
	return false;
}
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:false" title="" style="overflow-x: hidden;overflow-y: auto;">
		<form:form id="repayForm" method="post" modelAttribute="settleIntention" cssClass="form">
		...
		<table id="repayTable" class="common_table">
			...
			<tr>
				...
				<th class="narrow">豁免金额：</th>
				<td>
					<input type="text" class="text easyui-numberbox" value="0"
						id="exemptSum" name="exemptSum" data-rule="豁免金额:required;number;" 
				    	data-options="min:0.00,precision:2,max:${exemptAbleSum}" readonly='true'/>
				    <input type="hidden" value="${exemptAbleSum}"
						id="exemptAbleSum" name="exemptAbleSum" readonly='true'/>
				    <input type="hidden" value="${settleSum}"
						id="settleSum" name="settleSum" readonly='true'/>
				</td>
				<th>&nbsp;</th>
			</tr>
			<tr>
				...
				<td>
					<input type="text" class="text easyui-numberbox" value="${settleSum }"
						id="repayAmount" name="repayAmount" data-rule="交易金额:required;number;" 
			...
			<tr>
				<td colspan="2" style="vertical-align: top;">
					<table>
						...
						<tr>
							<th class="narrow" width="50%">购置款:</th>
							<td width="50%">
								<input id="assignAble_DK_GZK" name="assignAble['DK_GZK']" value = "0.00" readonly="true" class="text numberInput assignAble"/>
							</td>
						</tr>
						<tr>
							<th width="50%">超额还款:</th>
				...
				<td colspan="3" style="vertical-align: top;">
					<table>
						...
						<tr>
							<th class="narrow" colspan="2">豁免分配</th>
							<th>当期应收|总额应收</th>
						</tr>
						<tr>
						...
						<tr>
							<c:set var="sumTemp" value="${paymentScheduleEntryMap['DK_GZK'].receivableAmount + paymentScheduleEntryMap['DK_GZK'].exemptAmount - paymentScheduleEntryMap['DK_GZK'].actualReceivedAmount}"></c:set>
							<c:set var="gzkSum" value="${handler.getSum('MAIN','DK_GZK')}"></c:set>
							<th>购置款:</th>
							<td>
								<input id="assignIntention_DK_GZK" name="feeCfgItem.feeItem" value = ""
									<c:if test="${sumTemp < 0.01 && gzkSum < 0.01}">readonly='true'</c:if>
									class="text numberInput autoAssignItem easyui-numberbox"
									period="${paymentScheduleEntryMap['DK_GZK'].period}" 
									feeItem="DK_GZK" MONTHLY_REPAY_SUM="false"
									data-options="min:0,precision:2,max:${sumTemp}"/>
							</td>
							<th>
								<fmt:formatNumber value="${sumTemp}" pattern="#,##0.00#"/> |
								<fmt:formatNumber value="${gzkSum}" pattern="#,##0.00#"/>
							</th>
						</tr>
					</table>
				</td>
				<td colspan="2" style="vertical-align: top;">
					<table>
						...
						<tr>
							<th class="narrow" width="50%">购置款:</th>
							<td width="50%">
								<input id="assignView_DK_GZK" name="assignView['DK_GZK']" feeItem="DK_GZK" value = "0.00" readonly="true" class="text numberInput assignView"/>
							</td>
						</tr>
						<tr>
							<th width="50%">超额还款:</th>
