...
$("#payment_schedule").unbind('firstSelected').bind("firstSelected",function(){
	...
	paymentScheduleGrid = $('#paymentScheduleGrid').datagrid({
		...
		columns: [[
		{
			field : 'receivableSumIrr',
			title : '递减应收',
			width : 120
			,formatter: function(value, row, index){
				if(!row['paymentScheduleMap']){
					return 0;
				}
				var entry = row['paymentScheduleMap']['IRR'];
				if(entry){
					return entry.receivableSum;
				}
				return 0;
			}
		},
		{
			field : 'actualReceivedSumIrr',
			title : '递减实收',
			width : 120
			,formatter: function(value, row, index){
				if(!row['paymentScheduleMap']){
				...
				var entry = row['paymentScheduleMap']['IRR'];
				if(entry){
					return entry.actualReceivedSum;
				}
				return 0;
			}
		},
		{
			...
			,formatter: function(value, row, index){
				...
				var entry = row['paymentScheduleMap']['IRR'];
				if(entry){
					return entry.exemptSum;
				}
				return 0;
			}
		},
		{
			field : 'principalIrrReceivableAmountIrr',
			title : '本金递减应收',
			width : 100
			,formatter: function(value, row, index){
				if(!row || !row['paymentScheduleMap']){
					return 0;
				}
				var entryMap = row['paymentScheduleMap']['IRR'];
				if(!entryMap){
					return 0;
				}
				var entry = entryMap['paymentScheduleEntryMap']["DK_BJ"];
				if(entry){
					return entry.receivableAmount;
				}
				return 0;
	 	...
	 	{
			field : 'principalIrrActualReceivedAmountIrr',
			title : '本金递减实收',
			width : 100
			,formatter: function(value, row, index){
				if(!row || !row['paymentScheduleMap']){
		...
		{
			field : 'interestReceivableAmountIrr',
			title : '利息递减应收',
			width : 100
			,formatter: function(value, row, index){
				if(!row || !row['paymentScheduleMap']){
				...
				var entry = entryMap['paymentScheduleEntryMap']["DK_LX"];
				if(entry){
					return entry.receivableAmount;
				}
				return 0;
	 	...
	 	{
			field : 'interestActualReceivedAmountIrr',
			title : '利息递减实收',
			width : 100
			,formatter: function(value, row, index){
				if(!row || !row['paymentScheduleMap']){
		...
		{
			field : 'managementReceivableAmountIrr',
			title : '管理费递减应收',
			width : 100
			,formatter: function(value, row, index){
				if(!row || !row['paymentScheduleMap']){
				...
				var entry = entryMap['paymentScheduleEntryMap']["ZX_GLF"];
				if(entry){
					return entry.receivableAmount;
				}
				return 0;
		...
		{
			field : 'managementActualReceivedAmountIrr',
			title : '管理费递减实收',
			width : 100
			,formatter: function(value, row, index){
				if(!row || !row['paymentScheduleMap']){
		...
		{
			field : 'managementExemptAmountIrr',
			title : '管理费递减豁免',
			width : 100
			,formatter: function(value, row, index){
				if(!row || !row['paymentScheduleMap']){
