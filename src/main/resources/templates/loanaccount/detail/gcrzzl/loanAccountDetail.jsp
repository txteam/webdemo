						...
						<tr valign="top">
							...
							<td width="24%" style="vertical-align: top;">
								<table>
									...
									<tr>
										<th class="narrow">购置款：<span class="rightFloat">+</span></th>
										<td  align="right">
											<span id="settleReceivableSum_GZK">
												<fmt:formatNumber value="${loanAccountSettleDetail.getSum('MAIN','DK_GZK')}" pattern="#,##0.00#"/>
											</span>
										</td>
									</tr>
									<tr>
			...
			$.each(data,function(index,record){
				...
				if(!$.ObjectUtils.isEmpty(record.paymentRecordList) &&
					...
					$.each(record.paymentRecordList,function(index,item){
						if(item.period == 'NA' && item.scheduleType.key == 'MAIN'){
   		...
   		},
  		{
	 	...
	 	},
   		{
			...
			formatter: function(value, row, index){
				...
				if(row.paymentRecordEntryMap["DK_GLF"]){
					var entry = row.paymentRecordEntryMap["DK_GLF"];
					value += entry.amount ? entry.amount : 0;
				}
				return value;
   			}
   		...
   		},
  		{
   		...
   		},
   		{
   			...
   			,formatter: function(value, row, index){
				...
				if(row.paymentRecordEntryMap["ZX_TQHKWYJ"]){
					var entry = row.paymentRecordEntryMap["ZX_TQHKWYJ"];
					value += entry.amount ? entry.amount : 0;
				}
				return value;
   			}
