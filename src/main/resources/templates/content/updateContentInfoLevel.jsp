	...
	$('#entityForm').validator({
	    valid: function(){
			...
			$('#entityForm').ajaxSubmit({
			    url:"${contextPath }/contentInfoLevel/update.action",
			    success: function(data) {
			    	DialogUtils.progress('close');
					if(data){
						parent.DialogUtils.tip("更新内容等级成功");
						parent.DialogUtils.closeDialogById("updateContentInfoLevel");
					}else{
						parent.DialogUtils.tip("更新内容等级失败");
					}
			    } 
			});
			return false;
		...
		<form:form id="entityForm" method="post" cssClass="form"
			...
			<table class="common_table">
				<tr>
					...
					<td width="80%">
						<form:input path="code" cssClass="text" disabled="true"
							data-rule="编码:required;remote[${contextPath}/contentInfoLevel/validateCodeIsExist.action, code, id]" 
						/>
					</td>
				</tr>
				<tr>
					<th class="narrow" width="20%">内容信息类型</th>
					<td width="80%">
						<form:select path="category.code" disabled="true"
							items="${categories }" itemLabel="name" itemValue="code"></form:select>
					</td>
				</tr>
				<tr>
