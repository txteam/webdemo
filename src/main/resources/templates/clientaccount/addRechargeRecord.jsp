...
$(document).ready(function(){
	...
	//验证器
	$('#rechargeRecordForm').validator({
	    valid: function(){
	        //表单验证通过，提交表单到服务器
	        DialogUtils.progress({
	        	text : '数据提交中，请等待....'
	        });
			$('#rechargeRecordForm').ajaxSubmit({
			    url:"${contextPath }/rechargeRecord/addRechargeRecord.action",
			    success: function(data) {
			    	DialogUtils.progress('close');
					if(data){
						parent.DialogUtils.tip("新增rechargeRecord成功");
						parent.DialogUtils.closeDialogById("addRechargeRecord");
					}else{
						parent.DialogUtils.tip("新增rechargeRecord失败");
					}
			    } 
			});
			return false;
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'center',border:false" title="" style="overflow: hidden;">
		<form:form id="rechargeRecordForm" method="post" cssClass="form"
			modelAttribute="rechargeRecord">
			<form:hidden path="id"/>
				...
				<tr>
					...
					<td width="80%">
						<form:input path="serialNumber" cssClass="text" 
							data-rule="serialNumber:required;;remote[${contextPath}/rechargeRecord/validateSerialNumberIsExist.action, serialNumber, id]" 
						/>
					</td>
				</tr>
				<tr>
