...
function toAdjust(id,name) {
	...
	DialogUtils.openModalDialog(
			"distribute",
			$.formatString("充值申请单审批至分配[{0}]",name),
			$.formatString("${contextPath}/rechargeApplyRecord/toQueryClientInfoForDistribute.action?status=APPROVE_TO_ADJUST&rechargeApplyId={0}",id),
			1000,520,function(){
				grid.datagrid('load',$('#queryForm').serializeObject());
			});
	return false;
...
<body>
	<div class="easyui-layout" data-options="fit : true,border : false">
			...
			<form id="queryForm" class="form">
				<table class="table table-hover table-condensed">
					<tr>
						...
						<th hidden=true>充值渠道:</th>
						<td hidden=true>
							<select id="paymentChannel" name="paymentChannel">
								<option value="">--- 不限 ---</option>
								<c:forEach items="${paymentChannelEnums }" var="temp">
									<option value="${temp.key }" <c:if test="${temp.key == paymentChannel }">selected</c:if> >--- ${temp.name } ---</option>
								</c:forEach>
		...
		<div id="toolbar" style="display: none;">
			...
			<%--<c:if test='${authContext.hasAuth("approve_to_adjust") }'>
				<a id="toAdjust" onclick="toAdjust();" href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:true,iconCls:'pencil_add'">挂账撤销</a>
			</c:if>--%>
