...
<title>queryNoticeMessagePagedList</title>
...
$.canUpdate = false;
$.canDelete = false;
<c:if test='${authContext.hasAuth("add_noticeMessage")}'>
	$.canAdd = true;
</c:if>
<c:if test='${authContext.hasAuth("delete_noticeMessage")}'>
	$.canDelete = true;
</c:if>
<c:if test='${authContext.hasAuth("update_noticeMessage")}'>
	$.canUpdate = true;
</c:if>
...
$(document).ready(function(){
	...
	grid = $('#grid').datagrid({
		url : '${contextPath}/noticeMessage/queryPagedList.action',
		fit : true,
		fitColumns : true,
		border : false,
		idField : 'id',
		...
		{
			<%!//FIXME: 修改属性需要显示的属性字段 --%>
			field : 'noticeMessageType.id',
			<%!//FIXME: 修改属性中文名 --%>
			title : 'noticeMessageType.id',
			width : 200
		},
		{
			field : 'priority',
...
function addFun() {
	...
	DialogUtils.openModalDialog(
		"addNoticeMessage",
		"新增" + entityName,
		$.formatString("${contextPath}/noticeMessage/toAddNoticeMessage.action"),
		450,220,function(){
			queryFun();
	});
	return false;
...
function editFun(id,name) {
	...
	DialogUtils.openModalDialog(
		"updateNoticeMessage",
		"编辑" + entityName + ":" + name,
		$.formatString("${contextPath}/noticeMessage/toUpdateNoticeMessage.action?noticeMessageId={0}",id),
		450,220,function(){
			queryFun();
	});
	return false;
	...
	DialogUtils.confirm("确认提醒" , 
    	...
    	function(data){
	    	if(data){
	    		...
	    		//如果确认删除指定的NoticeMessage
	    		$.post(
			    		'${contextPath}/noticeMessage/deleteById.action',
			    		{noticeMessageId:id},
			    		function(data){
...
<div class="easyui-layout" data-options="fit : true,border : false">
	...
	<div id="toolbar" style="display: none;">		
		...
		<c:if test='${authContext.hasAuth("enable_noticeMessage") }'>
		...
		<c:if test='${authContext.hasAuth("disable_noticeMessage") }'>
