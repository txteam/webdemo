	...
	$('#entityForm').validator({
	    valid: function(){
			...
			$('#entityForm').ajaxSubmit({
			    url:"${contextPath }/noticeMessageType/add.action",
			    success: function(data) {
			    	DialogUtils.progress('close');
					if(data){
						parent.DialogUtils.tip("新增站内消息类型成功.");
						parent.DialogUtils.closeDialogById("addNoticeMessageType");
					}else{
						DialogUtils.alert("错误提示","新增站内消息类型失败.","error");
					}
			    } 
			});
			return false;
