...
<title>queryNoticeMessage2ClientPagedList</title>
...
$.canUpdate = false;
$.canDelete = false;
<c:if test='${authContext.hasAuth("add_noticeMessage2Client")}'>
	$.canAdd = true;
</c:if>
<c:if test='${authContext.hasAuth("delete_noticeMessage2Client")}'>
	$.canDelete = true;
</c:if>
<c:if test='${authContext.hasAuth("update_noticeMessage2Client")}'>
	$.canUpdate = true;
</c:if>
...
<%!//FIXME: 修改实体名称 --%>
var entityName = 'NoticeMessage2Client'; 
...
$(document).ready(function(){
	...
	grid = $('#grid').datagrid({
		url : '${contextPath}/noticeMessage2Client/queryPagedList.action',
		fit : true,
		fitColumns : true,
		border : false,
		idField : 'id',
		...
		{
			field : 'noticeMessageId',
			<%!//FIXME: 修改属性中文名 --%>
			title : 'noticeMessageId',
			width : 200
		},
		{
			field : 'clientType',
...
function addFun() {
	...
	DialogUtils.openModalDialog(
		"addNoticeMessage2Client",
		"新增" + entityName,
		$.formatString("${contextPath}/noticeMessage2Client/toAddNoticeMessage2Client.action"),
		450,220,function(){
			queryFun();
	});
	return false;
...
function editFun(id,name) {
	...
	DialogUtils.openModalDialog(
		"updateNoticeMessage2Client",
		"编辑" + entityName + ":" + name,
		$.formatString("${contextPath}/noticeMessage2Client/toUpdateNoticeMessage2Client.action?noticeMessage2ClientId={0}",id),
		450,220,function(){
			queryFun();
	});
	return false;
...
function deleteFun(id,name) {
	...
	//判断是否确认删除指定的NoticeMessage2Client
	DialogUtils.confirm("确认提醒" , 
    	...
    	function(data){
	    	if(data){
	    		...
	    		//如果确认删除指定的NoticeMessage2Client
	    		$.post(
			    		'${contextPath}/noticeMessage2Client/deleteById.action',
			    		{noticeMessage2ClientId:id},
			    		function(data){
...
<div class="easyui-layout" data-options="fit : true,border : false">
	...
	<div id="toolbar" style="display: none;">		
		...
		<c:if test='${authContext.hasAuth("enable_noticeMessage2Client") }'>
		...
		<c:if test='${authContext.hasAuth("disable_noticeMessage2Client") }'>
