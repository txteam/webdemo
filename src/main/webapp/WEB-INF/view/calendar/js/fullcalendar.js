...
(function($) {
	...
	$.fn.fullCalendar = function(options) {
		...
		if (typeof options == 'string') {
			...
			this
					.each(function() {
						var r = $.data(this, 'fullCalendar')[options].apply(
								this, args);
						if (res == undefined) {
							res = r;
						}
		...
		options = $
				.extend(true, {}, defaults,
						(options.isRTL || options.isRTL == undefined
								&& defaults.isRTL) ? rtlDefaults : {}, options);
		...
		this
				.each(function() {
					...
					// element
					var _element = this, element = $(_element).addClass('fc'), elementWidth, content = $(
							"<div class='fc-content "
									+ tm
									+ "-widget-content' style='position:relative'/>")
							.prependTo(_element), // relative for ie6
					contentHeight;
					if (options.isRTL) {
					...
					// view managing
					var date = new Date(), viewName, view, // the current view
					viewInstances = {};
					...
					function changeView(v) {
						if (v != viewName) {
							...
							if (header) {
								...
								header.find('div.fc-button-' + viewName)
										.removeClass(tm + '-state-active');
								header.find('div.fc-button-' + v).addClass(
										tm + '-state-active');
							}
					...
					function render(inc, forceUpdateSize) {
						if ((elementWidth = _element.offsetWidth) !== 0) { // visible
																			// on
																			// the
																			// screen
							if (!contentHeight) {
							...
							if (inc || !view.date || +view.date != +date) { // !view.date
																			// means
																			// it
																			// hasn't
																			// been
								...
								view
										.render(
												...
												contentHeight,
												function(callback) {
													// dont refetch if new view
													// contains the same events
													// (or a subset)
													if (!eventStart
								...
								// ensure events are rerendered if another view
								// messed with them
								// pass in 'events' b/c event might have been
								// added/removed
								view.clearEvents();
							...
							if (header) {
								// update title text
								header.find('h2.fc-header-title').html(
										view.title);
								// enable/disable 'today' button
								var today = new Date();
								if (today >= view.start && today < view.end) {
									header.find('div.fc-button-today')
											.addClass(tm + '-state-disabled');
								} else {
									header
											.find('div.fc-button-today')
					...
					// called when any event objects have been
					// added/removed/changed, rerenders
					...
					function fetchEvents(callback) {
						...
						var queued = eventSources.length, sourceDone = function() {
							if (--queued == 0) {
								if (callback) {
									callback(events);
								}
					...
					// Fetch from a particular source. Append to the 'events'
					// array
					function fetchEventSource(src, callback) {
						var prevViewName = view.name, prevDate = cloneDate(date), reportEvents = function(
								a) {
							if (prevViewName == view.name && +prevDate == +date
									&& // protects from fast switching
									$.inArray(src, eventSources) != -1) { // makes
																			// sure
																			// source
																			// hasn't
																			// been
						...
						if (typeof src == 'string') {
							var params = {};
							params[options.startParam] = Math.round(eventStart
									.getTime() / 1000);
							params[options.endParam] = Math.round(eventEnd
									.getTime() / 1000);
							if (options.cacheParam) {
								params[options.cacheParam] = (new Date())
										.getTime(); // TODO: deprecate
													// cacheParam
							}
							pushLoading();
							$.ajax({
								...
								dataType : 'json',
								data : params,
								cache : options.cacheParam || false, // don't
																		// let
																		// jquery
																		// prevent
																		// caching
					...
					var publicMethods = {
						...
						gotoDate : function(year, month, dateNum) {
							if (typeof year == 'object') {
								date = cloneDate(year); // provided 1 argument,
														// a Date
							} else {
								if (year != undefined) {
						...
						updateEvent : function(event) { // update an existing
														// event
							var i, len = events.length, e, startDelta = event.start
									- event._start, endDelta = event.end ? (event.end - (event._end || view
									.defaultEventEnd(event))) // event._end
																// would be null
																// if event.end
									: 0; // was null and event was just
											// resized
							for (i = 0; i < len; i++) {
								e = events[i];
						...
						renderEvent : function(event, stick) { // render a new
																// event
							normalizeEvent(event, options);
							if (!event.source) {
								if (stick) {
									(event.source = eventSources[0])
											.push(event);
								}
								events.push(event);
							}
										...
										$("<tr/>")
												.append(
														$(
																...
																.append(
																		buildSection(sections.left)))
												.append(
														$(
																"<td class='fc-header-center'/>")
																.append(
																		buildSection(sections.center)))
												.append(
														$(
																"<td class='fc-header-right'/>")
																.append(
																		buildSection(sections.right))))
								.prependTo(element);
					}
																...
																function(j,
																	...
																	if (buttonName == 'title') {
																		...
																		if (buttonClick) {
																			...
																			var button, icon = options.theme ? smartProperty(
																					options.buttonIcons,
																					buttonName)
																					: null, text = smartProperty(
																					options.buttonText,
																					buttonName);
																			if (icon) {
																				button = $("<div class='fc-button-"
																						+ buttonName
																						+ " ui-state-default'>"
																						+ "<a><span class='ui-icon ui-icon-"
																						+ icon
																						+ "'/></a></div>");
																			} else if (text) {
																				button = $("<div class='fc-button-"
					...
					var contentSizeFixed = false, resizeCnt = 0;
					...
					function fixContentSize() {
						if (!contentSizeFixed) {
							...
							// TODO: previous action might have caused
							// scrollbars
							// which will make the window width more narrow,
					...
					function unfixContentSize() {
						if (contentSizeFixed) {
							...
							if ($.browser.msie
								...
								// in IE6/7 the inside of the content div was
								// invisible
								// bizarre hack to get this work... need both
								// lines
					...
					function windowResize() {
						if (!contentSizeFixed) {
							if (view.date) { // view has already been
												// rendered
								...
								// was probably in a 0x0 iframe that has just
								// been resized
							}
						}
					}
					...
					// in IE, when in 0x0 iframe, initial resize never gets
					// called, so do this...
	...
	function normalizeEvent(event, options) {
		event._id = event._id
				|| (event.id == undefined ? '_fc' + fakeID++ : event.id + '');
		if (event.date) {
			if (!event.start) {
	...
	// TODO: if there is no title or start date, return false to indicate an
	// invalid event
		...
		return new Grid(
				...
				options,
				{
					render : function(date, delta, height, fetchEvents) {
						...
						// visStart/visEnd
						var visStart = this.visStart = cloneDate(start), visEnd = this.visEnd = cloneDate(this.end), nwe = options.weekends ? 0
								: 1;
						if (nwe) {
							skipWeekend(visStart);
							skipWeekend(visEnd, -1, true);
						}
						addDays(visStart, -((visStart.getDay()
								- Math.max(options.firstDay, nwe) + 7) % 7));
						addDays(visEnd, (7 - visEnd.getDay() + Math.max(
								options.firstDay, nwe)) % 7);
						// row count
						var rowCnt = Math.round((visEnd - visStart)
								/ (DAY_MS * 7));
						...
						// title
						this.title = formatDate(start, this
								.option('titleFormat'), options);
						// render
						this.renderGrid(rowCnt, options.weekends ? 7 : 5, this
								.option('columnFormat'), true, height,
								fetchEvents);
					}
				});
	}
		...
		return new Grid(
				...
				options,
				{
					render : function(date, delta, height, fetchEvents) {
						...
						var visStart = this.visStart = cloneDate(this.start = addDays(
								cloneDate(date), -((date.getDay()
										- options.firstDay + 7) % 7))), visEnd = this.visEnd = cloneDate(this.end = addDays(
								cloneDate(visStart), 7));
						if (!options.weekends) {
							skipWeekend(visStart);
							skipWeekend(visEnd, -1, true);
						}
						this.title = formatDates(visStart, addDays(
								cloneDate(visEnd), -1), this
								.option('titleFormat'), options);
						this.renderGrid(1, options.weekends ? 7 : 5, this
								.option('columnFormat'), false, height,
								fetchEvents);
					}
				});
	};
	...
	function Grid(element, options, methods) {
		...
		var tm, firstDay, nwe, // no weekends (int)
		rtl, dis, dit, // day index sign / translate
		rowCnt, colCnt, colWidth, thead, tbody, cachedSegs = [], // ...
		...
		view = $.extend(this, viewMethods, methods,
				{
					...
					defaultEventEnd : function(event) { // calculates an end if
														// event doesnt have
														// one, mostly for
														// resizing
						return cloneDate(event.start);
					},
					visEventEnd : function(event) { // returns exclusive
													// 'visible' end, for
													// rendering
						if (event.end) {
							var end = cloneDate(event.end);
							return (event.allDay || end.getHours() || end
									.getMinutes()) ? addDays(end, 1) : end;
						} else {
		...
		function renderGrid(r, c, colFormat, showNumbers, height, fetchEvents) {
			...
			var month = view.start.getMonth(), today = clearTime(new Date()), s, i, j, d = cloneDate(view.visStart);
			...
			if (!tbody) { // first time, build all cells from scratch
				...
				s = "<thead><tr>";
				for (i = 0; i < colCnt; i++) {
					s += "<th class='fc-" + dayIDs[d.getDay()] + ' ' + // needs
																		// to be
																		// first
					tm + '-state-default' + (i == dit ? ' fc-leftmost' : '')
				...
				if (rowCnt < prevRowCnt) {
					tbody.find('tr:gt(' + (rowCnt - 1) + ')').remove(); // remove
																		// extra
																		// rows
				} else if (rowCnt > prevRowCnt) { // needs to create new
													// rows...
					s = '';
				...
				tbody.find('td').each(
						function() {
							...
							if (+d == +today) {
								td.removeClass('fc-not-today').addClass(
										'fc-today').addClass(
										tm + '-state-highlight');
							} else {
								td.addClass('fc-not-today').removeClass(
		...
		function dayClick(ev) {
			var n = parseInt(this.className.match(/fc\-day(\d+)/)[1]), date = addDays(
					cloneDate(view.visStart), Math.floor(n / colCnt) * 7 + n
							% colCnt);
		...
		function updateSize(height) {
			...
			var leftTDs = tbody.find('tr td:first-child'), tbodyHeight = height
					- thead.height(), rowHeight1, rowHeight2;

			if (options.weekMode == 'variable') {
				rowHeight1 = rowHeight2 = Math.floor(tbodyHeight
						/ (rowCnt == 1 ? 2 : 6));
			} else {
			...
			if (tdHeightBug == undefined) {
				...
				var tr = tbody.find('tr:first'), td = tr.find('td:first');
			...
			setOuterWidth(thead.find('th').slice(0, -1), colWidth = Math
					.floor(element.width() / colCnt));
		...
		function compileSegs(events) {
			var d1 = cloneDate(view.visStart), d2 = addDays(cloneDate(d1),
					colCnt), rows = [], i = 0;
			for (; i < rowCnt; i++) {
		...
		function renderSegs(segRows) {
			var i, len = segRows.length, levels, tr, td, innerDiv, top, rowContentHeight, j, segs, levelHeight, k, seg, event, className, startElm, endElm, left, right, eventElement, eventAnchor, triggerRes;
			for (i = 0; i < len; i++) {
				levels = segRows[i];
				tr = tbody.find('tr:eq(' + i + ')');
				td = tr.find('td:first');
				innerDiv = td.find('div.fc-day-content div').css('position',
						'relative');
				...
				for (j = 0; j < levels.length; j++) {
					...
					for (k = 0; k < segs.length; k++) {
						...
						startElm = seg.isStart ? tr
								.find('td:eq('
										+ ((seg.start.getDay()
												- Math.max(firstDay, nwe) + colCnt) % colCnt)
										+ ') div div')
								: tbody;
						endElm = seg.isEnd ? tr.find('td:eq('
								+ ((seg.end.getDay() - Math.max(firstDay, nwe)
										+ colCnt - 1) % colCnt) + ') div div')
								: tbody;
						if (rtl) {
						...
						if (triggerRes !== false) {
							...
							if (rtl && rtlLeftDiff == undefined) {
								// bug in IE6 where offsets are miscalculated
								// with direction:rtl
								rtlLeftDiff = left
										- eventElement.position().left;
								if (rtlLeftDiff) {
									eventElement
											.css('left', left + rtlLeftDiff);
								}
							}
							...
							if (event.editable || event.editable == undefined
								...
								if (seg.isEnd) {
									view.resizableDayEvent(event, eventElement,
											colWidth);
								}
							}
							...
							view.trigger('eventAfterRender', event, event,
									eventElement);
							levelHeight = Math.max(levelHeight, eventElement
									.outerHeight(true));
						}
					}
				...
				eventElement.draggable({
					...
					stop : function(ev, ui) {
						...
						if (!cell || !cell.rowDelta && !cell.colDelta) {
							...
							eventElement.find('a').removeAttr('href'); // prevents
																		// safari
																		// from
																		// visiting
																		// the
																		// link
							view.eventDrop(this, event, cell.rowDelta * 7
									+ cell.colDelta * dis, 0, event.allDay, ev,
									ui);
						}
					}
				});
				...
				options,
				{
					render : function(date, delta, height, fetchEvents) {
						...
						this.title = formatDates(visStart, addDays(
								cloneDate(visEnd), -1), this
								.option('titleFormat'), options);
						this.renderAgenda(options.weekends ? 7 : 5, this
								.option('columnFormat'), height, fetchEvents);
					}
				});
	};
	...
	views.agendaDay = function(element, options) {
		return new Agenda(element, options, {
			render : function(date, delta, height, fetchEvents) {
				...
				this.start = this.visStart = cloneDate(date, true);
				this.end = this.visEnd = addDays(cloneDate(this.start), 1);
				this.renderAgenda(1, this.option('columnFormat'), height,
						fetchEvents);
			}
		});
	};
	...
	function Agenda(element, options, methods) {
		...
		var head, body, bodyContent, bodyTable, bg, colCnt, axisWidth, colWidth, slotHeight, cachedDaySegs = [], cachedSlotSegs = [], cachedHeight, tm, firstDay, nwe, // no
																																										// weekends
																																										// (int)
		rtl, dis, dit, // day index sign / translate
		minMinute, maxMinute,
		...
		function renderAgenda(c, colFormat, height, fetchEvents) {
			...
			var d0 = rtl ? addDays(cloneDate(view.visEnd), -1)
					: cloneDate(view.visStart), d = cloneDate(d0), today = clearTime(new Date());
			...
			if (!head) { // first time rendering, build from scratch
				...
				if (options.allDaySlot) {
					s += "<tr class='fc-all-day'>"
							...
							+ tm
							+ "-state-default'>&nbsp;</th>"
							+ "</tr><tr class='fc-divider fc-last'><th colspan='"
							+ (colCnt + 2) + "' class='" + tm
				...
				for (i = 0; d < maxd; i++) {
					...
					s += "<tr class='"
							+ (i == 0 ? 'fc-first' : (minutes == 0 ? ''
									: 'fc-minor'))
							+ "'><th class='fc-axis fc-leftmost "
							+ tm
							+ "-state-default'>"
							+ ((!slotNormal || minutes == 0) ? formatDate(d,
									options.axisFormat) : '&nbsp;')
							+ "</th><td class='fc-slot" + i + ' ' + tm
							+ "-state-default'><div>&nbsp;</div></td></tr>";
				...
				// redo column header text and class
				head.find('tr:first th').slice(1, -1)
						.each(
								function() {
									$(this).text(
											formatDate(d, colFormat, options));
		...
		function resetScroll() {
			...
			var go = function() {
				body.scrollTop(timePosition(d0, scrollDate) + 1); // +1 for
																	// the
																	// border
		...
		function updateSize(height) {
			...
			var topTDs = head.find('tr:first th'), stripeTDs = bg.find('td'), contentWidth = body[0].clientWidth;
			bodyTable.width(contentWidth);

			// time-axis width
			axisWidth = 0;
			setOuterWidth(head.find('tr:lt(2) th:first').add(
					body.find('tr:first th')).width('').each(function() {
			...
			setOuterWidth(topTDs.slice(-2, -1), contentWidth - axisWidth
					- colWidth * (colCnt - 1));
		...
		function slotClick(ev) {
			var col = Math.floor((ev.pageX - bg.offset().left) / colWidth), date = addDays(
					cloneDate(view.visStart), dit + dis * col), rowMatch = this.className
					.match(/fc-slot(\d+)/);
			if (rowMatch) {
				var mins = parseInt(rowMatch[1]) * options.slotMinutes, hours = Math
						.floor(mins / 60);
				date.setHours(hours);
		...
		function renderEvents(events) {
			...
			renderDaySegs(cachedDaySegs = stackSegs(view.sliceSegs(dayEvents,
		...
		function compileSlotSegs(events) {
			var d = addMinutes(cloneDate(view.visStart), minMinute), levels, segCols = [], i = 0;
			for (; i < colCnt; i++) {
				levels = stackSegs(view.sliceSegs(events, d, addMinutes(
						cloneDate(d), maxMinute - minMinute)));
		...
		function renderDaySegs(segRow) {
			if (options.allDaySlot) {
				var td = head.find('td'), tdInner = td.find('div div'), tr = td
						.parent(), top = safePosition(tdInner, td, tr, tr
						.parent()).top, rowContentHeight = 0, i, len = segRow.length, level, levelHeight, j, seg, event, className, leftDay, leftRounded, rightDay, rightRounded, left, right, eventElement, anchorElement, triggerRes;
				for (i = 0; i < len; i++) {
					level = segRow[i];
					levelHeight = 0;
				...
				updateSize(cachedHeight); // tdInner might have pushed the
											// body down, so resize
			}
		}
		...
		function renderSlotSegs(segCols) {
			var colI, colLen = segCols.length, col, levelI, level, segI, seg, forward, event, top, bottom, tdInner, width, left, className, eventElement, anchorElement, timeElement, titleElement, triggerRes;
			for (colI = 0; colI < colLen; colI++) {
				...
				for (levelI = 0; levelI < col.length; levelI++) {
					...
					for (segI = 0; segI < level.length; segI++) {
						...
						tdInner = bg.find('td:eq(' + (colI * dis + dit)
								+ ') div div');
						...
						if (levelI) {
							...
							if (forward) {
								...
								width = ((availWidth / (forward + 1)) - (12 / 2)) * 2; // 12
																						// is
																						// the
																						// predicted
																						// width
						...
						left = axisWidth + tdInner.position().left + // leftmost
																		// possible
						...
						if (triggerRes !== false) {
							...
							if (eventElement.height()
									- titleElement.position().top < 10) {
								// event title doesn't have enough room, put
								// next to the time
								timeElement.text(formatDate(event.start, view
										.option('timeFormat'))
										+ ' - ' + event.title);
								titleElement.remove();
							}
							...
							if (event.editable || event.editable == undefined
									&& options.editable) {
								draggableSlotEvent(event, eventElement,
										timeElement);
								if (seg.isEnd) {
									resizableSlotEvent(event, eventElement,
											timeElement);
								}
							}
						}
		...
		function draggableDayEvent(event, eventElement, isStart) {
			if (!options.disableDragging && eventElement.draggable) {
				var origPosition, origWidth, resetElement, allDay = true, matrix;
				eventElement
						.draggable({
							zIndex : 9,
							...
							start : function(ev, ui) {
								view.hideEvents(event, eventElement);
								view.trigger('eventDragStart', eventElement,
										event, ev, ui);
								origPosition = eventElement.position();
								origWidth = eventElement.width();
								resetElement = function() {
									if (!allDay) {
										eventElement.width(origWidth)
												.height('').draggable('option',
														'grid', null);
										allDay = true;
									}
								};
								matrix = new HoverMatrix(
										function(cell) {
												...
												if (!cell.row) { // on
													...
													if (isStart && allDay) {
														...
														setOuterHeight(
																...
																slotHeight
																		* Math
																				.round((event.end ? ((event.end - event.start) / MINUTE_MS)
																						: options.defaultEventMinutes)
																						/ options.slotMinutes));
														eventElement
																.draggable(
							...
							stop : function(ev, ui) {
								view.hideOverlay();
								view.trigger('eventDragStop', eventElement,
										event, ev, ui);
								var cell = matrix.cell, dayDelta = dis
										* (allDay ? // can't trust cell.colDelta
													// when using slot grid
										(cell ? cell.colDelta : 0)
												: Math
														.floor((ui.position.left - origPosition.left)
																/ colWidth));
									...
									view
											.eventDrop(
													...
													allDay ? 0
															...
															Math
																	.round((eventElement
																			.offset().top - bodyContent
																			.offset().top)
																			/ slotHeight)
																	* options.slotMinutes
																	+ minMinute
																	- (event.start
																			.getHours() * 60 + event.start
																			.getMinutes()),
													allDay, ev, ui);
								}
							}
		...
		function draggableSlotEvent(event, eventElement, timeElement) {
			if (!options.disableDragging && eventElement.draggable) {
				var origPosition, resetElement, prevSlotDelta, slotDelta, allDay = false, matrix;
				eventElement
						.draggable({
							zIndex : 9,
							scroll : false,
							...
							start : function(ev, ui) {
								...
								view.trigger('eventDragStart', eventElement,
										event, ev, ui);
								if ($.browser.msie) {
									eventElement.find('span.fc-event-bg')
											.hide(); // nested opacities mess
														// up in IE, just hide
								}
								origPosition = eventElement.position();
								resetElement = function() {
							...
							drag : function(ev, ui) {
								slotDelta = Math
										.round((ui.position.top - origPosition.top)
												/ slotHeight);
								if (slotDelta != prevSlotDelta) {
									if (!allDay) {
										// update time header
										var minuteDelta = slotDelta
												* options.slotMinutes, newStart = addMinutes(
								...
								if (!cell || !slotDelta && !dayDelta) {
									...
									if ($.browser.msie) {
										eventElement.css('filter', '') // clear
																		...
																		// opacity
																		// side-effects
										.find('span.fc-event-bg').css(
												'display', ''); // .show() made
																// display=inline
									}
									eventElement.css(origPosition); // sometimes
																	// fast
																	// drags
																	// make
																	// event
									...
									view.eventDrop(this, event, dayDelta,
											allDay ? 0 : slotDelta
													* options.slotMinutes, // minute
																			// delta
											allDay, ev, ui);
								}
							}
						...
						.resizable(
								{
									...
									start : function(ev, ui) {
										...
										if ($.browser.msie
												&& $.browser.version == '6.0') {
											eventElement.css('overflow',
													'hidden');
										}
										eventElement.css('z-index', 9);
										view.trigger('eventResizeStart', this,
												event, ev, ui);
									},
									resize : function(ev, ui) {
										// don't rely on ui.size.height, doesn't
										// take grid into account
										slotDelta = Math
												.round((Math.max(slotHeight,
														eventElement.height()) - ui.originalSize.height)
														/ slotHeight);
										if (slotDelta != prevSlotDelta) {
											timeElement
													.text(formatDates(
									...
									stop : function(ev, ui) {
										view.trigger('eventResizeStop', this,
												event, ev, ui);
										if (slotDelta) {
											view
													.eventResize(
											...
											view
													.showEvents(event,
															eventElement);
											// BUG: if event was really short,
											// need to put title back in span
										}
									}
		...
		// objects)
		function timePosition(day, time) { // both date object. day holds 00:00
											// of current day
			...
			var slotMinutes = options.slotMinutes, minutes = time.getHours()
					* 60 + time.getMinutes() - minMinute, slotI = Math
					.floor(minutes / slotMinutes), tr = body.find('tr:eq('
					+ slotI + ')'), td = tr.find('td'), innerDiv = td
					.find('div');
			return Math
					.max(0, Math.round(safePosition(innerDiv, td, tr, tr
							.parent()).top
							- 1
							+ slotHeight
							* ((minutes % slotMinutes) / slotMinutes)));
		}

	}

	// count the number of colliding, higher-level segments (for event
	// squishing)
			...
			for (j = 0; j < level.length; j++) {
				...
				for (k = 0; k < levels[i - 1].length; k++) {
					...
					if (segsCollide(segForward, segBack)) {
						segBack.forward = Math.max(segBack.forward || 0,
	...
	var viewMethods = {
		 ...
		 * properties/methods: - title - start - end - visStart - visEnd -
		 * defaultEventEnd(event) - visEventEnd(event) - render(events) -
		 * rerenderEvents()
		 * 
		 * 
		 * z-index reservations: 3 - day-overlay 8 - events 9 -
		...
		trigger : function(name, thisObj) {
			if (this.options[name]) {
				return this.options[name].apply(thisObj || this,
						Array.prototype.slice.call(arguments, 2).concat(
								[ this ]));
			}
		},
		...
		eventEnd : function(event) {
			return event.end ? cloneDate(event.end) : this
					.defaultEventEnd(event); // TODO: make sure always using
												// copies
		},

		// report when view receives new events

		reportEvents : function(events) { // events are already normalized at
											// this point
			var i, len = events.length, event, eventsByID = this.eventsByID = {}, cachedEvents = this.cachedEvents = [];
			for (i = 0; i < len; i++) {
				event = events[i];
		...
		_eee : function(event, exceptElement, funcName) { // event-element-each
			var elements = this.eventElementsByID[event._id], i, len = elements.length;
			for (i = 0; i < len; i++) {
		...
		eventDrop : function(e, event, dayDelta, minuteDelta, allDay, ev, ui) {
			var view = this, oldAllDay = event.allDay;
			view.moveEvents(view.eventsByID[event._id], dayDelta, minuteDelta,
					allDay);
			view.trigger('eventDrop', e, event, dayDelta, minuteDelta, allDay,
					function() { // TODO: change docs
						// TODO: investigate cases where this inverse technique
						// might not work
						view.moveEvents(view.eventsByID[event._id], -dayDelta,
								-minuteDelta, oldAllDay);
						view.rerenderEvents();
					}, ev, ui);
		...
		eventResize : function(e, event, dayDelta, minuteDelta, ev, ui) {
			var view = this;
			view.elongateEvents(view.eventsByID[event._id], dayDelta,
					minuteDelta);
			view.trigger('eventResize', e, event, dayDelta, minuteDelta,
					function() {
						// TODO: investigate cases where this inverse technique
						// might not work
						view.elongateEvents(view.eventsByID[event._id],
		...
		moveEvents : function(events, dayDelta, minuteDelta, allDay) {
			...
			for ( var e, len = events.length, i = 0; i < len; i++) {
				...
				if (e.end) {
					e.end = addMinutes(addDays(e.end, dayDelta, true),
							minuteDelta);
				}
			...
			if (!view.options.disableResizing && eventElement.resizable) {
				eventElement
						.resizable({
							...
							grid : colWidth,
							minWidth : colWidth / 2, // need this or else IE
														// throws errors when
														// too small
							containment : view.element.parent().parent(), // the
																			// main
																			// element...
							// ... a fix. wouldn't allow extending to last
							// column in agenda views (jq ui bug?)
							start : function(ev, ui) {
								eventElement.css('z-index', 9);
								view.hideEvents(event, eventElement);
								view.trigger('eventResizeStart', this, event,
										ev, ui);
							},
							stop : function(ev, ui) {
								view.trigger('eventResizeStop', this, event,
										ev, ui);
								// ui.size.width wasn't working with grid
								// correctly, use .width()
								var dayDelta = Math
										.round((eventElement.width() - ui.originalSize.width)
												/ colWidth);
								if (dayDelta) {
									view.eventResize(this, event, dayDelta, 0,
											ev, ui);
								} else {
									eventElement.css('z-index', 8);
									view.showEvents(event, eventElement);
	...
	function fixDate(d, check) { // force d to be on check's YMD, for
									// daylight savings purposes
		if (+d) { // prevent infinite looping on invalid dates
	...
	function skipWeekend(date, inc, excl) {
		inc = inc || 1;
		while (date.getDay() == 0
				|| (excl && date.getDay() == 1 || !excl && date.getDay() == 6)) {
			addDays(date, inc);
		}
		return date;
	}
	...
	var parseISO8601 = fc.parseISO8601 = function(s, ignoreTimezone) {
		...
		var date = new Date(m[1], 0, 1), check = new Date(m[1], 0, 1, 9, 0), offset = 0;
		if (m[3]) {
			date.setMonth(m[3] - 1);
	...
	var formatDates = fc.formatDates = function(date1, date2, format, options) {
		...
		for (i = 0; i < len; i++) {
			...
			if (c == "'") {
				...
				for (i2 = i + 1; i2 < len; i2++) {
					if (format.charAt(i2) == ')') {
						var subres = formatDate(date, format.substring(i + 1,
								i2), options);
	...
	function reportTBody(tbody) {
		if (operaPositionBug == undefined) {
			operaPositionBug = tbody.position().top != tbody.find('tr')
					.position().top;
		}
	}
	...
	function safePosition(element, td, tr, tbody) {
		...
		if (operaPositionBug) {
			position.top += tbody.position().top + tr.position().top
					- td.position().top;
		}
		return position;
	}
	...
	function HoverMatrix(changeCallback) {
		...
		var tops = [], lefts = [], prevRowE, prevColE, origRow, origCol, currRow, currCol;

		this.row = function(e, topBug) {
			prevRowE = $(e);
			tops.push(prevRowE.offset().top
					+ ((operaPositionBug && prevRowE.is('tr')) ? prevRowE
							.parent().position().top : 0));
		};
	...
	var undefined, dayIDs = [ 'sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat' ];
