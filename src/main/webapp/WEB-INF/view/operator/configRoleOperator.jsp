...
OperatorChooseManager.prototype._loadChoosedOperatorIds = function(operatorIds,loadCallback){
	var _this = this;
	$.post("${contextPath}/operatorRef/queryChoosedOperatorIdSetByRoleId.action", 
			{'roleId' : roleId, 'operatorIds[]': operatorIds},function(data){
		loadCallback(data);
	});
};
...
function submitConfigPostOperator(){
	...
	$.post("${contextPath}/operatorRef/configRoleOperatorId.action",
			{roleId:roleId,selectedOperatorId:checkedOperatorIds,unSelectedOperatorId:unCheckedOperatorIds}, 
			function(data){
		DialogUtils.tip("配置角色人员成功");
