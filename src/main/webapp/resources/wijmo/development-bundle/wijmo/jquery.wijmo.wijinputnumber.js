...
(function (wijmo) {
    ...
    var wijinputnumber = (function (_super) {
        ...
        wijinputnumber.prototype._onTriggerClicked = function () {
        ...
        wijinputnumber.prototype._doSpin = function (up, repeating) {
            ...
            var rh = new wijmo.wijInputResult();
        ...
        wijinputnumber.prototype._deleteSelText = function (backspace) {
            ...
            if(!backspace) {
                sel = this.element.wijtextselection();
    ...
    var wijNumberTextProvider = (function (_super) {
        ...
        wijNumberTextProvider.prototype.insertAt = function (input, position, rh) {
            ...
            var endText;
            var newText;
            var oldGroupSeparatorCount;
            var newGroupSeparatorCount;
            var leftPrevCh;
            var leftCh;
                ...
                if(!$.wij.charValidator.isDigit(input)) {
                    ...
                    if(input !== ',' && input !== '.' && input !== ')' && input !== '+' && input !== '-' && input !== '(' && input !== this.getDecimalSeparator()) {
                        if(this._type === 'percent' && input === nf.percent.symbol) {
                            ...
                            if(this._type === 'currency' && input === nf.currency.symbol) {
                                rh.testPosition = position;
                                return true;
                            } else {
                                return false;
    ...
    var wijNumberFormat = (function () {
        ...
        wijNumberFormat.prototype.parseNumber = function (text) {
            if (typeof text === "undefined") { text = this._currentValueInString; }
            var arr = this._currentValueInString.split(this.decimalSeparator(), 2);
            return {
        ...
        wijNumberFormat.prototype._change = function (delta) {
            ...
            this._currentValueInString = "" + (num.decimal + delta) + this.decimalSeparator() + num.fraction;
        };
        ...
        wijNumberFormat.prototype.checkDigitsLimits = function (aDigitsCount) {
            try  {
                var arr = this._currentValueInString.split(this.decimalSeparator());
                var s;
                var d;
                var i;
                var ch;
        ...
        wijNumberFormat.prototype.checkMinValue = function (val, chkAndRepair, chkIsLessOrEqMin) {
            ...
            try  {
                arr = this._currentValueInString.split(this.decimalSeparator());
                ...
                val = '' + val + '';
                arr = val.split(this.decimalSeparator());
