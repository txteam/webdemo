...
(function ($) {
	...
	$.widget("wijmo.wijlightbox", {
		...
		_create: function () {
			...
			this.container = $('<div/>')
			    .addClass('wijmo-wijlightbox ui-widget' +
				' wijmo-wijlightbox-controls-' +
				o.controlsPosition)
			...
			this.frame = $('<div></div>')
			    .addClass('wijmo-wijlightbox-frame ui-widget-content' +
				' ui-corner-all ui-helper-clearfix')
				.appendTo(this.container);

			this.content = $('<div></div>')
			    .addClass('wijmo-wijlightbox-content')
				.appendTo(this.frame);
			...
			o.activeIndex = ((o.activeIndex >= 0 && o.groupItems[o.activeIndex]) ||
			...
			if (o.showControlsOnHover) {
				this.frame.bind({
					...
					'mousemove.wijlightbox': function () {
						if (self.container.data('moving.wijlightbox') === false &&
						self.container.data('accessvisible.wijlightbox') === false) {
		...
		_parseLink: function (a) {
			...
			o = this.options,
			$a = $(a),
			opt = {}, rel, $img, gallery, index,
			props = ['href', 'player', 'title', 'alt',
			'height', 'width', 'gallery', 'wijvideosrc'];
		...
		_parse: function () {
			...
			this.element
			.find('a[rel^="wijlightbox"], a[rel^="lightbox"]')
		...
		_createText: function (updateOnly) {
			...
			$text = $('<div></div>')
				.addClass('wijmo-wijlightbox-text wijmo-wijlightbox-text-' +
				cs + ' ui-widget-content ui-helper-clearfix');
			$title = $('<h3></h3>')
				.addClass('wijmo-wijlightbox-title wijmo-wijlightbox-title-' +
				cs + ' ui-helper-clearfix').appendTo($text);
			$detail = $('<p></p>')
				.addClass('wijmo-wijlightbox-detail wijmo-wijlightbox-detail-' +
				cs + ' ui-helper-clearfix').appendTo($text);

			if (o.textPosition === 'inside') {
				$text
					.addClass('ui-widget-content ui-helper-clearfix')
				...
				$text
					...
					.width(this.frame.width())
					.appendTo(this.container);
			} else if (o.textPosition === 'overlay' ||
			o.textPosition === 'titleOverlay') {
				$mask = $('<div></div>')
					.addClass('wijmo-wijlightbox-mask wijmo-wijlightbox-mask-' +
					cs + ' ui-helper-clearfix')
					.width(this.frame.width())
					.appendTo(this.frame);
				...
				if (o.textPosition === 'titleOverlay') {
					$text.bind({
						'mouseenter': function () {
							...
							var totalHeight = $title.outerHeight(true) +
							$detail.outerHeight(true),
								animOption = $.extend({},
								self._defaults.textShowOption,
								o.textShowOption,
								{ queue: false });
							$mask.animate({
								height: totalHeight
		...
		_removeText: function () {
			this.container.find(".wijmo-wijlightbox-text," +
			".wijmo-wijlightbox-mask").remove();
		},
		...
		_getActiveItem: function () {
			...
			o.groupItems.length &&
			o.activeIndex >= 0 &&
			o.activeIndex < o.groupItems.length) {
		...
		_refreshText: function (updateOnly) {
			var self = this, o = this.options, item,
			$text, $title, $detail, $mask;
			...
			self._defaults.textShowOption, o.textShowOption, { queue: false });
			if (o.textPosition === 'overlay' || o.textPosition === 'titleOverlay') {
				var toHeitht = titleHeight +
				(o.textPosition === 'overlay' ? detailHeight : 0);
				$text.height(0);
				$mask.height(0);
				$text.animate({
					height: toHeitht
		...
		_createNavButtons: function () {
			...
			if (!this.backBtn) {
				this.backBtn = $("<a href='#'/>")
					.addClass('wijmo-wijlightbox-button ' +
					'wijmo-wijlightbox-button-prev ui-state-default' +
					(o.controlsPosition === 'inside' ?
					' ui-corner-right' :
					' ui-corner-left'))
			...
			if (!this.nextBtn) {
				this.nextBtn = $("<a href='#'/>")
					.addClass('wijmo-wijlightbox-button ' +
					'wijmo-wijlightbox-button-next ui-state-default' +
					(o.controlsPosition === 'inside' ?
					' ui-corner-left' :
					' ui-corner-right'))
		...
		_refreshNavButtons: function () {
			...
			if (this.backBtn) {
				operation = o.activeIndex === 0 ?
				'addClass' :
				'removeClass';
				this.backBtn[operation]('ui-state-disabled');
			}

			if (this.nextBtn) {
				operation = o.activeIndex >= o.groupItems.length - 1 ?
				'addClass' :
				'removeClass';
				this.nextBtn[operation]('ui-state-disabled');
			}
		},
		...
		_hideToolboxButtons: function (anim) {
			this.container
			.find('.wijmo-wijlightbox-toolbox-button')
			.stop(true, true)
			.fadeOut(anim ? 600 : 0);
		},
		...
		_createCtrlButtons: function () {
			...
			$.each(buttons, function (index, name) {
				...
				if (name === 'play' && !this.playBtn) {
					self.playBtn = $("<a href='#'/>")
						.addClass('wijmo-wijlightbox-toolbox-button' +
					...
					self.stopBtn = $("<a href='#'/>")
						...
						'ui-state-default ui-state-disabled ui-corner-all')
			...
			$.each(buttons, function (index, name) {
				...
				if (name === 'close' && !this.closeBtn) {
					self.closeBtn = $("<a href='#'/>")
						.addClass('wijmo-wijlightbox-toolbox-button ' +
						'wijmo-wijlightbox-toolbox-button-close ' +
						'ui-state-default ui-corner-all')
					...
					self.fullBtn = $("<a href='#'/>")
						.addClass('wijmo-wijlightbox-toolbox-button ' +
						'wijmo-wijlightbox-toolbox-button-fullsize ' +
						'ui-state-default ui-corner-all')
		...
		_refreshDialogButtons: function () {
			if (this.fullBtn) {
				...
				if (icon) {
					...
					icon.addClass(this._isFullSize() ?
					'ui-icon-newwin' :
		...
		_createTimerBar: function () {
			...
			if (!self.timerBar) {
				self.timerBar = $("<div></div>")
					.addClass('wijmo-wijlightbox-timerbar ' +
					'ui-widget-content ui-priority-secondary ui-corner-all')
					.appendTo(self.toolBox);

				self.timerMeter = $("<div></div>")
					.addClass('wijmo-wijlightbox-timermeter ' +
					'ui-progressbar-value ui-widget-content ui-corner-all')
					.appendTo(self.toolBox);
			}
		},
		...
		_createCounter: function () {
			...
			if (!self.counter) {
				self.counter = $("<div></div>")
					.addClass('wijmo-wijlightbox-counter ui-state-default' +
					(o.controlsPosition === 'inside' ?
					'  ui-corner-tl' :
					' ui-corner-all'))
					.hide()
		...
		_refreshCounter: function (index) {
			...
			if (o.showCounter && o.groupItems.length > 1 && this.counter) {
				...
				if (o.counterType === "sequence") {
					...
					while (start !== end) {
						$("<a></a>")
							.addClass('wijmo-wijlightbox-counter-item ' +
							'ui-corner-all ui-state-default' +
							(start === index ?
							' wijmo-wijlightbox-counter-active ui-state-active' :
							''))
							.text(++start)
		...
		_hideCounter: function (anim) {
			this.container
				.find('.wijmo-wijlightbox-counter')
				.stop(true, true)
				.fadeOut(anim ? 600 : 0);
		},
		...
		_showOverlay: function () {
			...
			this.container.data('overlay.wijlightbox',
			new $.wijmo.wijlightbox.overlay(this));
		},
		...
		_showWaiting: function () {
			...
			this.container.data('waiting.wijlightbox',
			new $.wijmo.wijlightbox.overlay(this, true));
		},
		...
		_getUrl: function (item) {
			var o = this.options,
				url = (typeof item === 'string') ? item : item.href,
				rootUrl = o.rootUrl;
		...
		_preload: function () {
			...
			for (i = Math.max(0, o.activeIndex);
			i < Math.min(o.activeIndex + 5, o.groupItems.length);
			i++) {
		...
		_slideTo: function (prevPlayer, player, animation, next, complete) {
			var o = this.options, self = this,
			curImage = prevPlayer.getElement(),
			wrapper, slideContainer, direction,
			w, h, $img;

			if (curImage.parent().is('.wijmo-wijlightbox-aniwrapper')) {
				wrapper = curImage.parent();
			} else {
				wrapper = $.effects.createWrapper(curImage)
					.css({ overflow: 'hidden' });
		...
		_resize: function (complete) {
			var self = this,
			rect = $.extend({}, self.container.offset(), {
				width: self.frame.width(),
				height: self.frame.height()
			});
		...
		_size: function (size, calcOnly) {
			var o = this.options,
				width = o.width,
				height = o.height,
				playerWidth = o.autoSize ?
				(this.container.data('playerwidth.wijlightbox') || width) :
				width,
				playerHeight = o.autoSize ?
				(this.container.data('playerheight.wijlightbox') || height) :
				height, player,
				...
				$(document).bind('keydown.wijlightbox', $.proxy(self, '_keyDownHandler'))
					.bind('click.wijlightbox', function (e) {
						...
						if (self._isOpen() && !!o.closeOnOuterClick) {
							if (srcElement !== self.container.get(0) &&
							$(srcElement).parents().index(self.container) < 0) {
								self._close();
							}
						}
					});
		...
		_toggleFullSize: function () {
			...
			if (this._isFullSize()) {
				...
				self._resize(function () {
					self._adjustPlayerSize(self.content.innerWidth(),
					self.content.innerHeight());
				});
			} else {
		...
		_createPlayer: function (item, onload, onerror) {
			...
			var self = this, cached = false, player,
			handleLoad = function () {
				cached = true;
				this._hideWaiting();
			...
			player = new $.wijmo
			.wijlightbox[playerName](this, item, handleLoad, handlerError);
	...
	$.extend($.wijmo.wijlightbox, {
			...
			$('<img></img>')
				...
				.bind({
					'load': function () {
						var img = $(this);
						self.width =
					item.width ? parseInt(item.width, 10) : img.width();
						self.height =
					item.height ? parseInt(item.height, 10) : img.height();
						img.remove();
		...
		inline: function (lightbox, item, onload, onerror) {
			...
			if (item.height) {
				...
				var $temp = $(this.item.href).clone()
							.show()
							.append('<br clear="all" />')
							.wrap('<div class="wijmo-wijlightbox ui-widget">' +
							'<div class="wijmo-wijlightbox-frame ui-widget-content' +
							' ui-corner-all ui-helper-clearfix">' +
							'<div class="wijmo-wijlightbox-frame ui-widget-content' +
							' ui-corner-all ui-helper-clearfix">' +
							'<div class="wijmo-wijlightbox-content"> ' +
							'</div></div></div></div>')
		...
		wijvideo: function (lightbox, item, onload, onerror) {
			...
			this.item = item;
			this.url = this.lightbox._getUrl(item);
			this.wijvideosrc = (!!item.wijvideosrc && item.wijvideosrc !== "") ?
			this.lightbox._getUrl(item.wijvideosrc) :
			null;
			this.width = item.width ? parseInt(item.width, 10) : lo.width;
			this.height = item.height ? parseInt(item.height, 10) : lo.height;
	...
	$.extend($.wijmo.wijlightbox.img.prototype, {
		appendTo: function ($content, hidden) {
			...
			.addClass('wijmo-wijlightbox-image')
			.attr('src', this.url);
			if (hidden) {
				this.$element.hide();
			}
			if (this.lightbox.options.transAnimation &&
				this.lightbox.options.transAnimation.animated == "fade") {
				//fixed bug 20925
	...
	$.extend($.wijmo.wijlightbox.swf.prototype, {
		appendTo: function ($content) {
			...
			S.flash.embedSWF(this.item.href, this.id, '100%',
			'100%', version, express, flashvars, params);
		},
		remove: function () {
	...
	$.extend($.wijmo.wijlightbox.flv.prototype, {
		appendTo: function ($content) {
			...
			S.flash.embedSWF(swf, this.id, width, height,
			version, express, flashvars, params);
		},
		remove: function () {
			S.flash.expressInstallCallback();
			S.flash.removeSWF(this.id);
	...
	$.extend($.wijmo.wijlightbox, {
		overlay: function (lightbox, waiting) {
			this.mask = $('<div></div>')
				.addClass('ui-widget-overlay wijmo-wijlightbox-overlay' +
				(waiting ?
