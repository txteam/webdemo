...
(function ($) {
	...
	$.widget("wijmo.wijchartcore", {
	    options: {
			...
			// / visible:true, style:{fill:"#none", stroke:"none"}, textWidth: null,
						...
						if (legend.textWidth) {
							$.each(text, function (i, t) {
								if (series.visible === false && !isline) {
									$(t.node).data("hidden", true)
									.data("textOpacity", 
										t.attr("opacity") || 1);
									t.attr("opacity", 0.3);
								}

								$.wijraphael.addClass($(t.node), 
									"wijchart-legend-text wijchart-legend");
								$(t.node).data("legendIndex", legendIndex)
									.data("index", idx);
							});
						} else {
							if (series.visible === false && !isline) {
			...
			$.each(self.legends, function (idx, leg) {
				var bBox = leg.wijGetBBox(),
					...
					y = top + offsetY + bBox.height / 2 + textMargin.top + 
						maxHeight / 2 - bBox.height / 2,
					iconY = y - icon.wijGetBBox().height / 2, 
					chtStyle, legCover, leg0 = leg;
				...
				if (legend.textWidth) {
					leg.transform(Raphael.format("...T{0},{1}", 
						x + iconWidth + legendMargin, y - bBox.height / 2));
				} else {
					leg.transform(Raphael.format("...T{0},{1}", 
...
//			}
		...
		_bindLegendEvents: function () {
			...
			// fixed issue 29653. when the legend element is not draw, 
			// the live method will lost effect. use delegate instead of it.
			element.delegate(".wijchart-legend", "click." + widgetName, function (e) {
...
//				});
		...
		_legendClick: function (obj) {
			...
			var self = this,
				...
				legendNode = l.textWidth ? 
					$(legend[0].node) : $(legend.node),
				idx = i, legendDot;
		...
		_unbindLiveEvents: function () {
			...
			element.undelegate(".wijchart-legend", "." + widgetName)
				.undelegate(".wijchart-legend", widgetName);
//			$(".wijchart-legend", element[0]).(widgetName)
//			// for jQuery 1.7.1
//			.die("." + widgetName);
