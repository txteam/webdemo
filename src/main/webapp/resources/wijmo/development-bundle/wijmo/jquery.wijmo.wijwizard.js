...
(function ($) {
	...
	$.widget("wijmo.wijwizard", {
				...
				if (bt === 'common') {
					this.backBtn =
						...
						.appendTo(this.buttons).bind({
							...
							'mousedown': function () {
								addState('active', $(this));
							},
							'mouseup': function () {
								removeState('active', $(this));
							}
						}).attr("role", "button");
			...
			if (init) {
				...
				if (this.list) {
					this.list
						.addClass('ui-widget ui-helper-reset' +
							' wijmo-wijwizard-steps ui-helper-clearfix')
						.attr("role", "tablist");
					this.lis.addClass('ui-widget-header ui-corner-all')
						.attr("role", "tab");
				}
				this.container = $('<div/>');
				this.container.addClass('wijmo-wijwizard-content ui-widget' +
					' ui-widget-content ui-corner-all');
				this.container.append(this.panels);
				this.container.appendTo(this.element);
				this.panels.addClass('wijmo-wijwizard-panel ui-widget-content')
					.attr("role", "tabpanel");
				...
				if (o.activeIndex === undefined) {
					...
					if (typeof o.activeIndex !== 'number' &&
							this.panels.filter('.wijmo-wijwizard-actived').length) {
						o.activeIndex = this.panels
							.index(this.panels.filter('.wijmo-wijwizard-actived'));
					}
				...
				if (o.activeIndex >= 0 && this.panels.length) {
					...
					this.panels.eq(o.activeIndex).removeClass('wijmo-wijwizard-hide')
						.addClass('wijmo-wijwizard-actived').attr('aria-hidden', false);
		...
		_refreshStep: function () {
			...
			if (this.lis) {
				this.lis.removeClass('ui-priority-primary')
					.addClass('ui-priority-secondary').attr('aria-selected', false);
				if (o.activeIndex >= 0 && o.activeIndex <= this.lis.length - 1) {
					if (this.lis) {
						this.lis.eq(o.activeIndex).removeClass('ui-priority-secondary')
							.addClass('ui-priority-primary').attr('aria-selected', true);
					}
					if (this.scrollWrap) {
			...
			if (this.buttons && !o.loop) {
				...
				this.nextBtn[o.activeIndex >= this.panels.length - 1 ?
					'addClass' : 'removeClass']('ui-state-disabled')
		...
		destroy: function () {
			...
			if (this.list) {
				this.list.removeClass('ui-widget ui-helper-reset' +
					' wijmo-wijwizard-steps ui-helper-clearfix')
					.removeAttr('role');
			}

			if (this.lis) {
				this.lis.removeClass('ui-widget-header ui-corner-all' +
					' ui-priority-primary ui-priority-secondary').removeAttr('role');
				this.lis.each(function () {
		...
		add: function (index, title, desc) {
			...
			var self = this, o = this.options,
				 $panel = $(o.panelTemplate || self._defaults.panelTemplate)
					.data('destroy.wijwizard', true),
				$li;
			$panel.addClass('wijmo-wijwizard-panel ui-widget-content' +
				' ui-corner-all wijmo-wijwizard-hide')
			...
			if (this.panels.length === 1) { // after pagelize
				...
				$panel.removeClass('wijmo-wijwizard-hide')
					.addClass('wijmo-wijwizard-actived')
		...
		show: function (index) {
			...
			$hide = this.panels.filter(':not(.wijmo-wijwizard-hide)');
		...
		load: function (index) {
			...
			var self = this, 
				...
				url = $.data(p, 'load.wijwizard'),
				spinner;

			this.abort();

			// not remote or from cache
			if (!url || this.element.queue("wijwizard").length !== 0 && 
					$.data(p, 'cache.wijwizard')) {
				this.element.dequeue("wijwizard");
				return;
			}
		...
		abort: function () {
			...
			this.element.queue("wijwizard", 
				this.element.queue("wijwizard").splice(-2, 2));
		...
		url: function (index, url) {
			...
			/// </param>
			this.panels.eq(index).removeData('cache.wijwizard')
				.data('load.wijwizard', url);
			return this;
		},
