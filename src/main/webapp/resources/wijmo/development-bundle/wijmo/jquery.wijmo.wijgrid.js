...
(function ($) {
	...
	$.widget("wijmo.wijgrid", {
		...
		_onMouseMove: function (args) {
			...
			if (!this._canInteract() || (frozener && frozener.inProgress())) {
				return;
			}
			...
			if (hoveredCell) {
				...
				if (!$hoveredRow.length || $hoveredRow.hasClass("wijmo-wijgrid-foorow") || !($hoveredRow.hasClass("wijmo-wijgrid-datarow") || $hoveredRow.hasClass("wijmo-wijgrid-headerrow"))) {
					return;
				}
			...
			this.currentCellEditEnd = function (grid, e) {
				...
				if (rowObj && rowObj.length) {
					...
					if (result) {
						...
						// move focus from editor to wijgrid before editor element will be deleted.
						// Note: does not work in pure IE8 (focus is moved to the body element after deletion), see the fix (#28263) below
		...
		frozener: function (wijgrid) {
			...
			function _showPosition(e) {
				...
				if (e.data) { // vertical
					...
					if ((element = _getFieldByPos({ x: e.pageX, y: e.pageY }))) { // get column widget
						...
						if (!$.ui.isOverAxis(position, _visibleBounds.left - 1, _visibleBounds.width + 2)) {
							return;
						}
					...
					if ((element = _getRowByPos({ x: e.pageX, y: e.pageY }))) {
						...
						if (!$.ui.isOverAxis(position, _visibleBounds.top - 1, _visibleBounds.height + 2)) {
							return;
						}
			...
			function _getFieldByPos(pos) {
				...
				for (i = 0, len = columns.length; i < len; i++) {
					...
					if (o.isLeaf) {
						...
						if ($.ui.isOverAxis(pos.x, bounds.left, bounds.width)) {
							return colWidget;
						}
					}
				}
			...
			function _getRowByPos(pos) {
				...
				for (i = 0, len = rows.length(); i < len; i++) {
					...
					if ($.ui.isOverAxis(pos.y, bounds.top, bounds.height)) {
						return row;
					}
				}
